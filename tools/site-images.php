...
class OftalmagSiteImages
{
	...
	public const PUBLIC_BASE = 'https://oftalmag.ru';
	...
	{
		$filename = 'oftalmag-images-' . date('Y-m-d-His') . '.xls';
...
if (!OftalmagSiteImages::isAllowed()) {
	...
	echo '<body style="font:14px/1.5 system-ui;padding:40px"><h1>Доступ запрещён</h1><p>'
		. htmlspecialchars(OftalmagSiteImages::accessDeniedReason())
		. '</p></body></html>';
	exit;
}
...
if ($action === 'download') {
	OftalmagSiteImages::downloadFile((int)($_GET['id'] ?? 0));
}
...
$limit = max(1, min(OftalmagSiteImages::PAGE_SIZE_MAX, (int)($_GET['limit'] ?? OftalmagSiteImages::PAGE_SIZE_DEFAULT)));
$offset = ($page - 1) * $limit;
$export = isset($_GET['export']) && $_GET['export'] === 'excel';

$result = OftalmagSiteImages::fetch([
	'q' => $q,
	'module' => $module,
	'usage' => $usage,
	'limit' => $limit,
...
if ($export) {
	OftalmagSiteImages::exportExcel($result['rows']);
}

$total = $result['total'];
$rows = $result['rows'];
$pages = max(1, (int)ceil($total / $limit));
$modules = OftalmagSiteImages::modulesList();
...
<head>
	...
	<title>Картинки сайта — oftalmag.ru</title>
	<style>
		:root {
			--bg: #f0f3f7;
			--card: #fff;
				...
				<tr>
					...
					<td>
						...
						<div style="color:var(--muted);font-size:12px;margin-top:4px">
							...
							<?=htmlspecialchars(OftalmagSiteImages::formatSize((int)$r['FILE_SIZE']))?>
