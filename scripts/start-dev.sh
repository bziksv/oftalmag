...
if mysqladmin --protocol=SOCKET ping --silent 2>/dev/null; then
  ...
  TABLES=$(mysql -h 127.0.0.1 -u oftalmag_local -poftalmag_local -N -e \
    "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='oftalmag_ru'" 2>/dev/null || echo 0)
  if [ "$TABLES" -lt 50 ]; then
    ...
    echo "Local MySQL: oftalmag_ru ($TABLES tables)"
  fi
else
