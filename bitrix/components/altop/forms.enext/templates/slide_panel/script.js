(function() {
	...
	if(!!window.JCFormsSlidePanelComponent)
		return;
	...
	window.JCFormsSlidePanelComponent = function(params) {
	...
	window.JCFormsSlidePanelComponent.prototype =	{		
		init: function() {
					...
					if(this.props.hasOwnProperty(i)) {
						...
						if(!!formInput) {
							...
							if(this.props[i].CODE == 'EMAIL') {
									fields[this.props[i].CODE].validators.regexp = {
										regexp: /^([a-z0-9_\-]+\.)*[a-z0-9_\-]+@([a-z0-9][a-z0-9\-]*[a-z0-9]\.)+[a-z]+[a-z]/i,
										message: BX.message('FORMS_EMAIL_INVALID')
									};
							}
						}
					}
