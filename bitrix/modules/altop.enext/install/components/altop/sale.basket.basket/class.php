...
class CBitrixBasketComponent extends CBitrixComponent {
	...
	public function onPrepareComponentParams($params) {
		...
		if(isset($params['CONTEXT_SITE_ID']) && $params['CONTEXT_SITE_ID'] > 0 && Bitrix\Main\Loader::includeModule('landing')) {
		...
		if(!isset($params['COLUMNS_LIST_EXT']) || !is_array($params['COLUMNS_LIST_EXT'])) {
			if(!empty($params['COLUMNS_LIST'])) {
				...
				$params['COLUMNS_LIST_EXT'] = [
					"NAME", "DISCOUNT", "WEIGHT", "DELETE", "DELAY", "TYPE", "PRICE", "QUANTITY", "SUM"
				];
			}
		}
		...
		if(empty($params['ACTION_VARIABLE']) || !preg_match('/[a-zA-Z0-9_-~.!*\'(),]/', $params['ACTION_VARIABLE'])) {
		...
		$params["DISABLE_BASKET"] = false;
		if($arSettings["DISABLE_BASKET"] == "Y")
			$params["DISABLE_BASKET"] = true;

		$params["DISABLE_DELAY"] = false;
		if($arSettings["DISABLE_DELAY"] == "Y")
			$params["DISABLE_DELAY"] = true;

		//clear basket//
		//clear delay//
		if($params["DISABLE_BASKET"] || $params["DISABLE_DELAY"]) {
			$basket = $this->getBasketStorage()->getBasket();
			if(!$basket->isEmpty()) {
				if($params["DISABLE_BASKET"]) {
				...
				foreach($basket as $basketItem) {
					if($basketItem->canBuy()) {
						if($params["DISABLE_BASKET"] && !$basketItem->isDelay()) {
							$basketItem->delete();
							$sessionBasketQuantity--;
						} elseif($params["DISABLE_DELAY"] && $basketItem->isDelay()) {
							$basketItem->delete();
						}
					}
				}
				...
				if($params["DISABLE_BASKET"])
					$this->setSessionFUserBasketQuantity($sessionBasketQuantity, $fUserId);
			}
		}
		
		$params["QUICK_ORDER"] = true;
		if($arSettings["QUICK_ORDER"] != "Y")
			$params["QUICK_ORDER"] = false;

		$params["DISABLE_ORDER"] = false;
		if($arSettings["DISABLE_ORDER"] == "Y")
			$params["DISABLE_ORDER"] = true;

		$params["MIN_ORDER_SUM"] = intval($arSettings["MIN_ORDER_SUM"]);
	...
	protected function initialLoadAction() {
		...
		if(!$this->arResult['OBJECTS']) {
			$message = $this->checkMinOrderSum($this->arResult['allSum'], $this->arResult['CURRENCY']);
			if(!empty($message))
				$this->arResult['WARNING_MESSAGE'][] = $message;
		}
	...
	protected function selectItemAction() {
		...
		if(!$result['BASKET_DATA']['OBJECTS']) {
			$message = $this->checkMinOrderSum($result['BASKET_DATA']['allSum'], $result['BASKET_DATA']['CURRENCY']);
			if(!empty($message))
	...
	//legacy method
	protected function clearDelayAction() {
		global $APPLICATION;
			...
			if(!$basket->isEmpty()) {
				...
				if($item) {
					if($item->isDelay() && $item->canBuy()) {
						$addResult = $item->setField('DELAY', 'N');
						if($addResult->isSuccess()) {
							...
							if($saveResult->isSuccess()) {
								...
								$addResult->addErrors($saveResult->getErrors());
							}

							if(!$addResult->isSuccess())
								$_SESSION['SALE_BASKET_MESSAGE'][] = Loc::getMessage('SBB_PRODUCT_NOT_AVAILABLE', array('#PRODUCT#' => $item->getField('NAME')));
						}
					} else {
						$_SESSION['SALE_BASKET_MESSAGE'][] = Loc::getMessage('SBB_PRODUCT_NOT_AVAILABLE', array('#PRODUCT#' => $item->getField('NAME')));
					}
	...
	protected function recalculateAjaxAction() {
		...
		if(!empty($result['APPLIED_DISCOUNT_IDS']) || implode(',', $result['APPLIED_DISCOUNT_IDS']) !== $this->request->get('lastAppliedDiscounts') || $this->request->get('fullRecalculation') === 'Y') {
			//reload all items
	...
	protected function refreshAjaxAction() {
		if($this->needBasketRefresh() && ($this->request->get('fullRecalculation') === 'Y' || $this->basketHasItemsToUpdate())) {
	...
	public function getBasketItems() {
		...
		//OLD_PRICE//
		$hasOldPrice = false;
		$basketBasePrice = $result["allSum"] + $result["DISCOUNT_PRICE_ALL"];		
		foreach ($result["GRID"]["ROWS"] as &$arItem) {
			if (!empty($arItem["PROPERTY_OLD_PRICE_VALUE"]) && $arItem["DISCOUNT_PRICE_PERCENT"] == 0) {
				if (!$hasOldPrice) {
					$hasOldPrice = true;
                }

                $oldPrice = str_replace(",", ".", $arItem["PROPERTY_OLD_PRICE_VALUE"]);

                $arItem["FULL_PRICE"] = Price::roundPrice($arItem["~PRICE_TYPE_ID"], $oldPrice, $arItem["CURRENCY"]);
                $arItem["FULL_PRICE_FORMATED"] = CCurrencyLang::CurrencyFormat(
                    $arItem["FULL_PRICE"],
                    $arItem["CURRENCY"],
                    true
                );

                $arItem["DISCOUNT_PRICE"] = Price::roundPrice(
                    $arItem["~PRICE_TYPE_ID"],
                    $arItem["FULL_PRICE"] - $arItem["PRICE"],
                    $arItem["CURRENCY"]
                );

                $arItem["DISCOUNT_PRICE_PERCENT"] = Sale\Discount::calculateDiscountPercent(
                    $arItem["FULL_PRICE"],
                    $arItem["DISCOUNT_PRICE"]
                );
                $arItem["DISCOUNT_PRICE_PERCENT_FORMATED"] = Sale\BasketItem::formatQuantity(
                    $arItem["DISCOUNT_PRICE_PERCENT"]
                ) . "%";

                $arItem["SUM_DISCOUNT_PRICE"] = $arItem["DISCOUNT_PRICE"] * $arItem["QUANTITY"];
                $arItem["SUM_DISCOUNT_PRICE_FORMATED"] = CCurrencyLang::CurrencyFormat(
                    $arItem["SUM_DISCOUNT_PRICE"],
                    $arItem["CURRENCY"],
                    true
                );

                $arItem["SUM_FULL_PRICE"] = $arItem["FULL_PRICE"] * $arItem["QUANTITY"];
                $arItem["SUM_FULL_PRICE_FORMATED"] = CCurrencyLang::CurrencyFormat(
                    $arItem["SUM_FULL_PRICE"],
                    $arItem["CURRENCY"],
                    true
                );

                $basketBasePrice += Price::roundPrice(
                    $arItem["~PRICE_TYPE_ID"],
                    $arItem["DISCOUNT_PRICE"] * $arItem["QUANTITY"],
                    $arItem["CURRENCY"]
                );
		...
		if ($hasOldPrice) {
			$result["PRICE_WITHOUT_DISCOUNT"] = CCurrencyLang::CurrencyFormat(
                $basketBasePrice,
                $result["CURRENCY"],
                true
            );
			$result["DISCOUNT_PRICE_ALL"] = PriceMaths::roundPrecision($basketBasePrice - $result["allSum"]);
			$result["DISCOUNT_PRICE_FORMATED"] = $result["DISCOUNT_PRICE_ALL_FORMATED"] = CCurrencyLang::CurrencyFormat(
		...
		//MEASURE//
		$measureCodes = $arMeasureList = array();
		foreach($result["GRID"]["ROWS"] as $arItem) {
			$measureCodes[] = $arItem["MEASURE"];
		}
		unset($arItem);
		
		if(count($measureCodes) > 0) {
			$rsMeasures = CCatalogMeasure::getList(array(), array("CODE" => array_unique($measureCodes)), false, false, array("CODE", "SYMBOL_INTL"));
			while($arMeasure = $rsMeasures->GetNext()) {
				$arMeasureList[$arMeasure["CODE"]] = $arMeasure["SYMBOL_INTL"];
			}
			unset($arMeasure, $rsMeasures);
			
			foreach($result["GRID"]["ROWS"] as &$arItem) {
				if(array_key_exists($arItem["MEASURE"], $arMeasureList))
					$arItem["MEASURE_SYMBOL_INTL"] = $arMeasureList[$arItem["MEASURE"]];
			}
			unset($arItem);
		}
		unset($arMeasureList, $measureCodes);
		
		//PC_SQ_M_PRICE_QUANTITY_MEASURE_RATIO//
		foreach($result["GRID"]["ROWS"] as &$arItem) {
			if(!empty($arItem["PROPERTY_M2_COUNT_VALUE"])) {
				$sqMCount = str_replace(",", ".", $arItem["PROPERTY_M2_COUNT_VALUE"]);
				if($arItem["MEASURE_SYMBOL_INTL"] == "pc. 1") {
					$arItem["SQ_M_PRICE"] = Price::roundPrice($arItem["~PRICE_TYPE_ID"], $arItem["PRICE"] * $sqMCount, $arItem["CURRENCY"]);	
					$arItem["SQ_M_PRICE_FORMATED"] = CCurrencyLang::CurrencyFormat($arItem["SQ_M_PRICE"], $arItem["CURRENCY"], true);
					$arItem["SQ_M_FULL_PRICE"] = Price::roundPrice($arItem["~PRICE_TYPE_ID"], $arItem["FULL_PRICE"] * $sqMCount, $arItem["CURRENCY"]);	
					$arItem["SQ_M_FULL_PRICE_FORMATED"] = CCurrencyLang::CurrencyFormat($arItem["SQ_M_FULL_PRICE"], $arItem["CURRENCY"], true);

					$SqMMeasureRatio = round($arItem["MEASURE_RATIO"] / $sqMCount, 4);
					
					$arItem["PC_QUANTITY"] = $arItem["QUANTITY"];
					$arItem["SQ_M_QUANTITY"] = round(($arItem["QUANTITY"] * $SqMMeasureRatio) / $arItem["MEASURE_RATIO"], 4);
					
					$arItem["PC_MEASURE_RATIO"] = $arItem["MEASURE_RATIO"];
					$arItem["SQ_M_MEASURE_RATIO"] = $SqMMeasureRatio;
				} elseif($arItem["MEASURE_SYMBOL_INTL"] == "m2") {
					$arItem["PC_QUANTITY"] = round($arItem["QUANTITY"] / $arItem["MEASURE_RATIO"]);
					$arItem["SQ_M_QUANTITY"] = $arItem["QUANTITY"];

					$arItem["PC_MEASURE_RATIO"] = 1;
					$arItem["SQ_M_MEASURE_RATIO"] = $arItem["MEASURE_RATIO"];
				}
			}
		}
		unset($arItem);
		...
		foreach($result["GRID"]["ROWS"] as $arItem) {
			if($arItem["DELAY"] == "N" && $arItem["CAN_BUY"] == "Y") {
				if(!empty($arItem["PROPERTY_OBJECT_VALUE"]))
					$objectsIds[] = $arItem["PROPERTY_OBJECT_VALUE"];
			}
		}
		unset($arItem);
		...
		if(!empty($objectsIds)) {
			...
			$rsElements = CIBlockElement::GetList(array(), array("ID" => array_unique($objectsIds)), false, false, array("ID", "IBLOCK_ID", "NAME", "PREVIEW_PICTURE", "PROPERTY_ADDRESS", "PROPERTY_SITE_ID"));
			while($arElement = $rsElements->GetNext()) {
				$result["OBJECTS"][$arElement["ID"]] = array(
					"ID" => $arElement["ID"],
					"NAME" => $arElement["NAME"],
			...
			foreach($result["GRID"]["ROWS"] as $arItem) {
				if($arItem["DELAY"] == "N" && $arItem["CAN_BUY"] == "Y") {
					if(!empty($arItem["PROPERTY_OBJECT_VALUE"])) {
						$result["OBJECTS"][$arItem["PROPERTY_OBJECT_VALUE"]]["ITEMS"][] = $arItem;
					} else {
						$result["OBJECTS"][0]["ITEMS"][] = $arItem;
					}
				}
			}
			unset($arItem);

			foreach($result["OBJECTS"] as &$arObject) {
				$basketPrice = 0;
				$basketBasePrice = 0;
				foreach($arObject["ITEMS"] as $arItem) {
					$basketPrice += $arItem["SUM_VALUE"];
					$basketBasePrice += $arItem["BASE_PRICE"] * $arItem["QUANTITY"];
				}
				unset($arItem);
				
				$arObject["allSum"] = PriceMaths::roundPrecision($basketPrice);
				$arObject["allSum_FORMATED"] = CCurrencyLang::CurrencyFormat($arObject["allSum"], $result["CURRENCY"], true);

				$arObject["PRICE_WITHOUT_DISCOUNT"] = CCurrencyLang::CurrencyFormat($basketBasePrice, $result["CURRENCY"], true);
				$arObject["DISCOUNT_PRICE_ALL"] = PriceMaths::roundPrecision($basketBasePrice - $basketPrice);
				$arObject["DISCOUNT_PRICE_FORMATED"] = $arObject["DISCOUNT_PRICE_ALL_FORMATED"] = CCurrencyLang::CurrencyFormat($arObject["DISCOUNT_PRICE_ALL"], $result["CURRENCY"], true);
			}
			unset($arObject);
		}
		unset($objectsIds);
		...
		foreach($this->basketItems as &$item) {
			...
			if((int)$parentId > 0) {
				foreach($this->arCustomSelectFields as $field) {
					...
					if((!isset($item[$fieldVal]) || empty($item[$fieldVal])) && (isset($productsData[$parentId][$fieldVal]) && !empty($productsData[$parentId][$fieldVal]))) {
			...
			if(empty($productsData[$productId]['PREVIEW_PICTURE']) && empty($productsData[$productId]['DETAIL_PICTURE']) && isset($additionalImages[$productId])) {
				$item['PREVIEW_PICTURE'] = $additionalImages[$productId];
			} elseif(empty($item['PREVIEW_PICTURE']) && empty($item['DETAIL_PICTURE']) && $additionalImages[$parentId]) {
		...
		foreach($calcResults['DISCOUNT_LIST'] as $discountData) {
			...
			if(isset($calcResults['FULL_DISCOUNT_LIST'][$discountId])) {
				...
				if(empty($appliedDiscounts[$discountId]['RESULT']['BASKET'])) {
					$appliedDiscounts[$discountId]['RESULT']['BASKET'] = array();
				}
	...
	public function getSkuPropsData($basketItems, $parents, $arSkuProps = array()) {
		...
		$offerList = CCatalogSku::getOffersList(
			...
			array('ID', 'IBLOCK_ID'),
			array('CODE' => array_merge($arSkuProps, array("OBJECT", "PARTNERS_URL")))
		);
		...
		if(!empty($offerList)) {
			foreach(array_keys($offerList) as $index) {
				...
				foreach(array_keys($offerList[$index]) as $offerId) {
					...
					$object = !empty($offer['PROPERTIES']['OBJECT']['VALUE']) ? true : false;
					if($object) {
						foreach($itemIds as $itemId) {
							if(in_array($itemId, array_keys($offerList[$index])))
								unset($itemIds[$itemId]);
						}
						unset($itemId);
						unset($offerList[$index]);
						break;
					}
					unset($object, $offer['PROPERTIES']['OBJECT']);

					unset($itemIds[$offerId]);
					
					$basePrice = CPrice::GetBasePrice($offerId);
					$partnersUrl = !empty($offer['PROPERTIES']['PARTNERS_URL']['VALUE']) ? true : false;
					
					if($basePrice['PRICE'] <= 0 || $partnersUrl)
						unset($offer);
					unset($partnersUrl, $basePrice, $offer['PROPERTIES']['PARTNERS_URL']);
							...
							if(self::$highLoadInclude) {
								...
								if($hlblock) {
									...
									while($arData = $rsData->fetch()) {
										...
										if(isset($fieldsList['UF_CODE']))
											$arValues['n'.$arData['ID']]['CODE'] = $arData['UF_CODE'];
									}
								}
							}
						}
				...
				if(!empty($arUsedValues)) {
					...
					foreach($arRes[$item['IBLOCK_ID']] as $propId => $arProp) {
						...
						foreach($arProp['VALUES'] as $valId => $arValue) {
							...
							if(($arProp['TYPE'] == 'L' && (in_array($arValue['NAME'], $arUsedValues[$arProp['CODE']]) || in_array(htmlspecialcharsEx($arValue['NAME']), $arUsedValues[$arProp['CODE']]))) || ($arProp['TYPE'] == 'E' && in_array($arValue['ID'], $arUsedValues[$arProp['CODE']])) || ($arProp['TYPE'] == 'S' && in_array($arValue['XML_ID'], $arUsedValues[$arProp['CODE']]))) {
					...
					if(!empty($itemActionData['POST_RESTORE']) && $this->arParams['SHOW_RESTORE'] === 'Y') {
						...
						if($item) {
							if(!empty($itemActionData['POST_DELETE']) && in_array('DELETE', $this->columns)) {
								...
								if(isset($itemActionData['POST_QUANTITY']) && !empty($itemsRatioData[$id]) && $item->getQuantity() != $itemActionData['POST_QUANTITY'] && in_array('QUANTITY', $this->columns)) {
									$this->processChangeQuantity($result, $itemsRatioData[$id], $itemActionData['POST_QUANTITY']);
								}

								if(isset($itemActionData['POST_DELAY']) && $item->getField('DELAY') !== $itemActionData['POST_DELAY'] && ($itemActionData['POST_DELAY'] === 'N' || in_array('DELAY', $this->columns))) {
	...
	public function checkQuantity($basketItemData, $desiredQuantity) {
		...
		if($this->quantityFloat === 'Y' || (isset($basketItemData['MEASURE_RATIO']) && (float)$basketItemData['MEASURE_RATIO'] > 0 && (float)$basketItemData['MEASURE_RATIO'] != (int)$basketItemData['MEASURE_RATIO'])) {
	...
	protected function selectOfferByProps($iblockId, $productId, $currentOfferId, array $propertyValues, array $properties) {
		...
		$offers = CCatalogSku::getOffersList(
			...
			array('CODE' => array_merge($codeList, array('PARTNERS_URL')))
		);
		...
		foreach(array_keys($offers[$productId]) as $offerId) {
			$offer = &$offers[$productId][$offerId];
	...
	protected function checkMinOrderSum($sum, $currency) {
		$message = '';
		
		if(!$this->arParams['DISABLE_BASKET'] && $this->arParams['MIN_ORDER_SUM'] > 0 && $sum < $this->arParams['MIN_ORDER_SUM']) {
			$message = Loc::getMessage('SBB_MIN_ORDER_SUM').' '.CCurrencyLang::CurrencyFormat($this->arParams['MIN_ORDER_SUM'], $currency, true);
		}
