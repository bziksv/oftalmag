...
<div class="slide-panel-basket-objects">	
	<?foreach($arResult["OBJECTS"] as $arObject) {?>
		<div class="slide-panel-basket-object" data-object-id="<?=intval($arObject['ID'])?>">
			<?if(intval($arObject["ID"]) > 0) {?>
				<div class="slide-panel-basket-object-info">
					<div class="slide-panel-basket-object-info-name"><?=$arObject["NAME"]?></div>
					<div class="slide-panel-basket-object-info-address"><i class="icon-map-marker"></i><span><?=$arObject["ADDRESS"]?></span></div>
				</div>
			<?}?>
			<div class="slide-panel-basket-object-items">
				<?foreach($arObject["ITEMS"] as $arItem) {?>
					<div class="slide-panel-basket-object-item-tr" data-id="<?=$arItem['ID']?>" data-item-measure="<?=$arItem['MEASURE_SYMBOL_INTL']?>" data-entity="row">
						<?foreach($arResult["GRID"]["HEADERS"] as $arHeader) {
							//ITEM//
							if($arHeader["id"] == "NAME") {?>									
								<div class="slide-panel-basket-object-item-td slide-panel-basket-item-item">
									<?//IMAGE//?>
							...
							<?//QUANTITY//
								...
								<div class="slide-panel-basket-object-item-td slide-panel-basket-item-quantity">
							...
							<?//SUM//
								...
								<div class="slide-panel-basket-object-item-td slide-panel-basket-item-sum">
						...
						//DELETE//
						if($bDeleteColumn) {?>
							<div class="slide-panel-basket-object-item-td slide-panel-basket-item-controls">
			...
			foreach($arResult["GRID"]["HEADERS"] as $arHeader) {
				if($arHeader["id"] == "SUM") {?>
					<div class="slide-panel-basket-object-cols">
						<div class="slide-panel-basket-object-col">
							<span class="slide-panel-basket-footer-total-all-sum" data-entity="allSum_<?=intval($arObject['ID'])?>"><?=$arObject["allSum_FORMATED"]?></span>
							<?$showTotalPrice = (float)$arObject["DISCOUNT_PRICE_ALL"] > 0;?>
							<span class="slide-panel-basket-footer-total-old-sum" data-entity="oldSum_<?=intval($arObject['ID'])?>" style="display: <?=($showTotalPrice ? 'block' : 'none');?>;"><?=($showTotalPrice ? $arObject["PRICE_WITHOUT_DISCOUNT"] : '');?></span>
							<span class="slide-panel-basket-footer-total-discount-sum" data-entity="discountSum_<?=intval($arObject['ID'])?>" style="display: <?=($showTotalPrice ? 'block' : 'none');?>;"><?=($showTotalPrice ? GetMessage("SBB_SLIDE_PANEL_TOTAL_DISCOUNT").' '.$arObject["DISCOUNT_PRICE_ALL_FORMATED"] : '');?></span>
						</div>
						<?if(!$arParams["DISABLE_ORDER"] && !empty($arObject["SITE_ID"]) && $arObject["SITE_ID"] != SITE_ID) {?>
							<div class="slide-panel-basket-object-col">
								<button type="button" class="btn btn-buy" data-entity="checkOutObject"><span><?=GetMessage("SBB_SLIDE_PANEL_ORDER_SHORT")?></span></button>
							</div>
						<?}?>
					</div>
				<?}
	...
	<div class="slide-panel-basket-object-footer slidePanelCartFooterRightIn" data-entity="basketFooter">
		<div class="slide-panel-basket-object-footer-row slide-panel-basket-object-footer-button<?=($arParams['QUICK_ORDER'] ? 's' : '')?>">
			...
			<?if($arParams["QUICK_ORDER"]) {?>
				<button type="button" class="btn btn-primary" data-entity="quickOrder" data-has-object="true"><span><?=GetMessage("SBB_SLIDE_PANEL_QUICK_ORDER")?></span></button>
			<?}?>
		</div>
	</div>
</div>