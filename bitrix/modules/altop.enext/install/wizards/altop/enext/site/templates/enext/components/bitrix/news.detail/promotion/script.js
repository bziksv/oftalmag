(function() {
	...
	if(!!window.JCNewsDetailPromo)
		return;

	window.JCNewsDetailPromo = function(arParams) {
		this.config = {
			PARAMS: ''
		};
	...
	window.JCNewsDetailPromo.prototype = {
		init: function() {
			...
			if(this.errorCode === 0) {
				this.obTabs = this.obItem.querySelector('.promotions-detail-tabs-container');
				this.obTabsBlock = !!this.obTabs && this.obTabs.querySelector('[data-entity="tabs"]');
				this.obTabContainers = this.obItem.querySelector('.promotions-detail-tabs-content');
				...
				this.itemTimer = this.obItem.querySelector('[data-entity="timer"]');
				if(!!this.itemTimer)
					this.initTimer();
				
				this.sectionsLinksContainer = this.obItem.querySelector('.promotions-detail-sections-links');
				if(!!this.sectionsLinksContainer) {
					this.sectionsLinksContainerHeight = 92;
					this.sectionsLinks = this.sectionsLinksContainer.querySelectorAll('.promotions-detail-section-link');
					if(!!this.sectionsLinks) {
						...
						this.sectionLinkBtn = BX.create('DIV', {
							props: {
								className: 'promotions-detail-section-link-btn-container'
							},
							children: [
								BX.create('DIV', {
									props: {
										className: 'promotions-detail-section-link promotions-detail-section-link-btn'
									},
									children: [
		...
		initTabs: function() {
			var tabsList = this.obTabs.querySelector('.promotions-detail-tabs-list'),
			...
			if(!!tabs.length > 0) {
				...
				$(tabsList).owlCarousel({								
					...
					navContainer: '.promotions-detail-tabs-scroll',
					dots: false,			
				});
		...
		initTimer: function() {
			var itemActiveToStr = this.itemTimer.getAttribute('data-active-to'),
				itemActiveTo = eval("(" + itemActiveToStr + ")");
					
			if(Object.keys(itemActiveTo).length > 0) {
				$(this.itemTimer).countdown({
					...
					padZeroes: true,
					expiryText: BX.message('PROMOTIONS_ITEM_DETAIL_COMPLETED'),
					alwaysExpire: true
				});
			}
		},
		...
		changeSectionLink: function(event) {
			...
			if(!BX.hasClass(BX.proxy_context, 'active') && sectionId) {
				var itemProductsContainer = this.obItem.querySelector('.promotions-detail-products');
				if(!!itemProductsContainer) {					
					itemProductsContainer.style.opacity = 0.2;
					BX.ajax({
						url: BX.message('PROMOTION_TEMPLATE_PATH') + '/ajax.php',
						method: 'POST',
						dataType: 'json',
						timeout: 60,
						data: {
			...
			if(BX.pos(this.sectionsLinks[Object.keys(this.sectionsLinks).length - 1], true).bottom > this.sectionsLinksContainerHeight) {
				if(!this.sectionLinkBtnAdjusted) {
					...
					if(!BX.hasClass(this.sectionsLinksContainer, 'active'))
						this.sectionLinkBtnSpan.innerHTML = window.innerWidth < 1043 ? BX.message('PROMOTIONS_ITEM_DETAIL_PRODUCTS_SECTIONS_ALL') : BX.message('PROMOTIONS_ITEM_DETAIL_PRODUCTS_SECTIONS_SHOW_ALL');
		...
		showHideSectionsLinks: function() {
			if(!BX.hasClass(this.sectionsLinksContainer, 'active')) {
				...
				this.sectionLinkBtnSpan.innerHTML = BX.message('PROMOTIONS_ITEM_DETAIL_PRODUCTS_SECTIONS_HIDE');
