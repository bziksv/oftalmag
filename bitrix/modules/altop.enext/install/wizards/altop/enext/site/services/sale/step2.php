...
if(COption::GetOptionString("enext", "wizard_installed", "N", WIZARD_SITE_ID) != "Y") {
	$locationGroupID = 0;
	$arLocationArr = array();
...
if(CModule::IncludeModule("subscribe")) {
	...
	if(!$rsRubric->Fetch()) {
		...
		$arFields = array(
			...
			"VISIBLE" => "Y",
			"FROM_FIELD" => COption::GetOptionString("main", "email_from", "info@".$_SERVER["SERVER_NAME"]),
...
if(IntVal($userGroupID) > 0) {
	...
	WizardServices::SetFilePermission(array($siteID, $SiteDir."/brands/"), array($userGroupID => "W"));
	WizardServices::SetFilePermission(array($siteID, $SiteDir."/catalog/"), array($userGroupID => "W"));
	WizardServices::SetFilePermission(array($siteID, $SiteDir."/gallery/"), array($userGroupID => "W"));
	...
	WizardServices::SetFilePermission(array($siteID, $SiteDir."/places/"), array($userGroupID => "W"));
	WizardServices::SetFilePermission(array($siteID, $SiteDir."/promotions/"), array($userGroupID => "W"));
	WizardServices::SetFilePermission(array($siteID, $SiteDir."/services/"), array($userGroupID => "W"));
}
COption::SetOptionString("enext", "wizard_installed", "Y", false, WIZARD_SITE_ID);?>