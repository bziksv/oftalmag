(function() {
	...
	if(!!window.JCNewsDetailCollection)
		return;
	
	window.JCNewsDetailCollection = function(arParams) {
		this.config = {
			PARAMS: ''
		};
	...
	window.JCNewsDetailCollection.prototype = {
		init: function() {
			...
			if(this.errorCode === 0) {
				this.obTabs = this.obItem.querySelector('.collections-detail-tabs-container');
				this.obTabsBlock = !!this.obTabs && this.obTabs.querySelector('[data-entity="tabs"]');
				this.obTabContainers = this.obItem.querySelector('.collections-detail-tabs-content');
				...
				var slider = this.obItem.querySelector('.collections-item-detail-slider');
				if(!!slider) {
					...
					$(slider).owlCarousel({
						...
						navContainer: '.collections-item-detail-slider-container',
						dots: false,
						responsive: {
							0: {
								stagePadding: 0,
				...
				this.sectionsLinksContainer = this.obItem.querySelector('.collections-detail-sections-links');
				if(!!this.sectionsLinksContainer) {
					this.sectionsLinksContainerHeight = 92;
					this.sectionsLinks = this.sectionsLinksContainer.querySelectorAll('.collections-detail-section-link');
					if(!!this.sectionsLinks) {
						...
						this.sectionLinkBtn = BX.create('DIV', {
							props: {
								className: 'collections-detail-section-link-btn-container'
							},
							children: [
								BX.create('DIV', {
									props: {
										className: 'collections-detail-section-link collections-detail-section-link-btn'
									},
									children: [
		...
		initTabs: function() {
			var tabsList = this.obTabs.querySelector('.collections-detail-tabs-list'),
			...
			if(!!tabs.length > 0) {
				...
				$(tabsList).owlCarousel({								
					...
					navContainer: '.collections-detail-tabs-scroll',
					dots: false,			
				});
		...
		changeSectionLink: function(event) {
			...
			if(!BX.hasClass(BX.proxy_context, 'active') && sectionId) {
				var itemProductsContainer = this.obItem.querySelector('.collections-detail-products');
				if(!!itemProductsContainer) {					
					itemProductsContainer.style.opacity = 0.2;
					BX.ajax({
						url: BX.message('COLLECTION_TEMPLATE_PATH') + '/ajax.php',
						method: 'POST',
						dataType: 'json',
						timeout: 60,
						data: {
			...
			if(BX.pos(this.sectionsLinks[Object.keys(this.sectionsLinks).length - 1], true).bottom > this.sectionsLinksContainerHeight) {
				if(!this.sectionLinkBtnAdjusted) {
					...
					if(!BX.hasClass(this.sectionsLinksContainer, 'active'))
						this.sectionLinkBtnSpan.innerHTML = window.innerWidth < 1043 ? BX.message('COLLECTIONS_ITEM_DETAIL_PRODUCTS_SECTIONS_ALL') : BX.message('COLLECTIONS_ITEM_DETAIL_PRODUCTS_SECTIONS_SHOW_ALL');
		...
		showHideSectionsLinks: function() {
			if(!BX.hasClass(this.sectionsLinksContainer, 'active')) {
				...
				this.sectionLinkBtnSpan.innerHTML = BX.message('COLLECTIONS_ITEM_DETAIL_PRODUCTS_SECTIONS_HIDE');
