...
if(!empty($arResult["ORDER"])) {?>
	<span class="alert alert-success">
		<?=Loc::getMessage("SOA_ORDER_SUC", array("#ORDER_DATE#" => $arResult["ORDER"]["DATE_INSERT"], "#ORDER_ID#" => $arResult["ORDER"]["ACCOUNT_NUMBER"]));
							...
							<table class="sale_order_full_table">
								<tr>
									<td class="ps_logo">
										...
										<?=CFile::ShowImage($arPaySystem["LOGOTIP"], 100, 100, "border=0\" style=\"max-width: 78px; max-height: 42px;\"", "", false);?>
	...
	$basketCount = (int)Bitrix\Sale\BasketComponentHelper::getFUserBasketQuantity(Bitrix\Sale\Fuser::getId(true), SITE_ID);
	if($basketCount > 0) {?>
		<div class="sale_order_to_basket">
			<span><?=Loc::getMessage("SOA_ORDER_TO_BASKET")?></span>
			<a class="btn btn-buy" href="<?=$arParams['PATH_TO_BASKET']?>" role="button"><span><?=Loc::getMessage("SOA_ORDER_BTN_TO_BASKET")?></span></a>
		</div>
	<?}
} else {?>
