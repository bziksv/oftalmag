...
if($IBLOCK_CATALOG_ID == false) {	
	...
	$rsIblock = CIBlock::GetList(array(), array("XML_ID" => "enext_advantages_".WIZARD_SITE_ID, "TYPE" => "references"));
	if($arIblock = $rsIblock->Fetch())
		CWizardUtil::ReplaceMacros($_SERVER["DOCUMENT_ROOT"].$iblockXMLFile, array("ENEXT_ADVANTAGES_IBLOCK_ID" => $arIblock["ID"]));
	unset($arIblock, $rsIblock);

	$rsIblock = CIBlock::GetList(array(), array("XML_ID" => "enext_faq_".WIZARD_SITE_ID, "TYPE" => "content"));
	if($arIblock = $rsIblock->Fetch())
		CWizardUtil::ReplaceMacros($_SERVER["DOCUMENT_ROOT"].$iblockXMLFile, array("ENEXT_FAQ_IBLOCK_ID" => $arIblock["ID"]));
	unset($arIblock, $rsIblock);
