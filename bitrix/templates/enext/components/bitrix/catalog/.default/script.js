(function() {
	...
	window.JCCatalogComponent.prototype = {
		init: function() {
			...
			this.sectionViewContainer = document.body.querySelector('[data-role="catalogSectionView"]');
			this.sectionViewPopup = this.sectionViewContainer && this.sectionViewContainer.querySelector('[data-role="dropdownContentView"]');
			if(this.sectionViewContainer && this.sectionViewPopup) {
				BX.bind(this.sectionViewContainer, 'click', BX.proxy(this.showSectionViewDropDownPopup, this));
				BX.bind(document, 'click', BX.proxy(this.hideSectionViewDropDownPopup, this));
			}
		...
		showSectionViewDropDownPopup: function() {
			if(BX.isNodeHidden(this.sectionViewPopup)) {
				BX.style(this.sectionViewPopup, 'display', '');
				BX.addClass(this.sectionViewContainer, 'active');
			} else {
				BX.style(this.sectionViewPopup, 'display', 'none');
				BX.removeClass(this.sectionViewContainer, 'active');
			}
		},

		hideSectionViewDropDownPopup: function(event) {			
			var target = BX.getEventTarget(event);
			if(!BX.findParent(target, {attr: {'data-role': 'catalogSectionView'}}, false) && target.getAttribute('data-role') != 'catalogSectionView') {
				BX.style(this.sectionViewPopup, 'display', 'none');
				BX.removeClass(this.sectionViewContainer, 'active');
			}
		},
