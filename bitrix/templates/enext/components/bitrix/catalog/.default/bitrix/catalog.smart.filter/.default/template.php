...
<div class="bx-filter">				
	...
	<div class="bx-filter-inner">
		<form name="<?=$arResult['FILTER_NAME'].'_form'?>" action="<?=$arResult['FORM_ACTION']?>" method="get" class="smartfilter">
			...
			//NOT_PRICES//
			foreach($arResult["ITEMS"] as $key => $arItem) {
				if (in_array($arItem["CODE"], $arParams["SMART_FILTER_USER_FIELD"]))
					continue;
			...
			<div class="bx-filter-button-box">
				<div class="bx-filter-block">
					<button class="btn btn-buy" type="submit" id="set_filter" name="set_filter"><span><?=GetMessage("CT_BCSF_SET_FILTER")?></span><span class="bx-filter-count" <?if(!isset($arResult["ELEMENT_COUNT"])){ echo 'style="display:none"'; }else{ echo 'style="display: inline-block;"';}?> ><?=intval($arResult["ELEMENT_COUNT"])?></span></button>
