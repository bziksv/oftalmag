...
if($isFilter) {
	...
	<?$APPLICATION->IncludeComponent("bitrix:catalog.smart.filter", "",
		array(
			...
			"SMART_FILTER_USER_FIELD" => $arCurSection["UF_SMART_FILTER"],
		),
		$component,
		array("HIDE_ICONS" => "Y")
	);?>
...
<div class="catalog-section-container">
	...
	if($isSort && ((!empty($collectionsIds) && $catalogView != "collections") || (!isset($collectionsIds) || empty($collectionsIds)))) {
		...
		if(empty($getView)) {
			$getView = $arCurSection["UF_PRODUCTS_VIEW"];
		} 
		
		if(empty($getView)) {
			$getView = $arSettings["PRODUCTS_VIEW"]["VALUE"];
		}
	}
							...
							<div class="catalog-section-panel-block<?=(!empty($collectionsIds) || $isFilter ? "" : (!empty($arAvailableSort) ? " catalog-section-panel-block-reverse" : ""))?>">
								...
								if(!empty($arAvailableSort)) {?>
									...
									<div class="catalog-section-sort-container">
										...
										<!-- select options -->
										<div class="catalog-section-sort" data-role="catalogSectionView">
											<div class="catalog-section-sort-block">
												<div class="catalog-section-sort-text">
											...
											<div class="catalog-section-sort-popup" data-role="dropdownContentView" style="display: none;">
												<ul>
													<li>
														<a rel="nofollow" href="<?=$APPLICATION->GetCurPageParam('view=CARD', array('view'))?>">Плитка</a>
													</li>
													<li>
														<a rel="nofollow" href="<?=$APPLICATION->GetCurPageParam('view=LIST', array('view'))?>">Список</a>
													</li>
													<li>
														<a rel="nofollow" href="<?=$APPLICATION->GetCurPageParam('view=PRICE', array('view'))?>">Прайс</a>
													</li>
												</ul>
											</div>
										</div>
	...
	if(!empty($collectionsIds) && $catalogView == "collections") {
		...
		$intSectionID = $APPLICATION->IncludeComponent("bitrix:catalog.section", "",
			array(
				...
				"PAGE_ELEMENT_COUNT" => $arCurSection["UF_PAGE_ELEMENT_COUNT"] ?? $arParams["PAGE_ELEMENT_COUNT"],
