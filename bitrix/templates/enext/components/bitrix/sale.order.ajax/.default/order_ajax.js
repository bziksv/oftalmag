...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters) {
			...
			this.initialCountry = parameters.initialCountry || '';
			this.params = parameters.params || {};
			this.signedParamsString = parameters.signedParamsString || '';
			this.currency = parameters.currency || '';
			this.siteId = parameters.siteId || '';
			this.defSiteId = parameters.defSiteId || '';
			this.objectId = parameters.objectId || false;
		...
		sendRequest: function(action, actionData) {
			...
			if(action === 'saveOrderAjax') {
				...
				if(!!form) {
					var inputSessid = form.querySelector('input[type=hidden][name=sessid]');
					if(!!inputSessid)
						inputSessid.value = BX.bitrix_sessid();

					if(!!this.params.PHONE_MASK) {
						var props = form.querySelectorAll('.bx-soa-customer-field[data-property-id-row]');
						for(var i = 0; i < props.length; i++) {
							var id = props[i].getAttribute('data-property-id-row');
							if(this.iti[id]) {
								var input = props[i].querySelector('input[type=text]');
								if(!!input)
									data[input.name] = this.iti[id].getNumber();
							}
						}
					}
				}
				...
				if(!!form) {
					...
					for(var i = 0; i < props.length; i++) {
						...
						if(this.iti[id]) {
							var input = props[i].querySelector('input[type=text]');
							if(!!input)
								data.order[input.name] = this.iti[id].getNumber();
						}
					}
				}
			}
		...
		saveOrderWithJson: function(result) {
			...
			if(result && result.order) {
				...
				if(result.REDIRECT_URL) {
					...
					if(this.siteId != this.defSiteId && this.objectId)
						BX.onGlobalCustomEvent('OnOrderSaved', [result.ID, result.REDIRECT_URL]);
					else
		...
		getErrorContainer: function(node) {
			...
			node.appendChild(
				BX.create('DIV', {props: {className: 'alert alert-error'}, style: {display: 'none'}})
			);
		},
		...
		showError: function(node, msg, border) {
			...
			var errorContainer = node.querySelector('.alert.alert-error'),
				animate;
		...
		showErrors: function(errors, scroll, showAll) {
			var errorNodes = this.orderBlockNode.querySelectorAll('div.alert.alert-error'),
		...
		showBlockErrors: function(node) {
			var errorNode = node.querySelector('div.alert.alert-error'),
				hiddenNode, errors;
		...
		scrollToError: function() {
			...
			for(i in sections) {
				if(sections.hasOwnProperty(i)) {
					errorNode = sections[i].querySelector('.alert.alert-error');
		...
		showWarnings: function() {
			...
			if(currentDelivery && currentDelivery.CALCULATE_ERRORS) {
				...
				warningString = this.params.MESS_DELIVERY_CALC_ERROR_TITLE;
				if(this.params.MESS_DELIVERY_CALC_ERROR_TEXT.length)
					warningString += '<br />' + this.params.MESS_DELIVERY_CALC_ERROR_TEXT;
		...
		showBlockWarning: function(node, warnings, hide) {
			var errorNode = node.querySelector('.alert.alert-error'),
				warnStr = '',
		...
		showPagination: function(entity, node) {
			...
			if(pagination.pages.length > 1) {
				if(pagination.pageNumber > 1) {	
					navigation.push(
						BX.create('LI', {
							...
							html: '<a href="">' + this.params.MESS_NAV_BACK + '</a>',
							events: {
								click: BX.proxy(this.doPagination, this)
							}
						})
					);
				}
				...
				for(i = 0; i < pagination.pages.length; i++) {
					...
					active = pageCounter == pagination.pageNumber ? 'active' : '';
					...
					navigation.push(
						BX.create('LI', {
							...
							html: !!active ? '<span>' + pageCounter  + '</span>' : '<a href="">' + pageCounter  + '</a>',
							events: {
								click: BX.proxy(this.doPagination, this)
							}
						})
				...
				if(pagination.pageNumber != pagination.pages.length) {
					navigation.push(
						BX.create('LI', {
							attrs: {
								'data-action': 'next',
							...
							html: '<a href="">' + this.params.MESS_NAV_FORWARD + '</a>',
							events: {
								click: BX.proxy(this.doPagination, this)
							}
						})
				...
				paginationNode = BX.create('UL', {
					props: {className: 'pagination bx-soa-pagination'},
					children: navigation
				});
				
				node.appendChild(BX.create('DIV', {props: {className: 'clr'}}));
		...
		doPagination: function(e) {
			...
			if(BX.hasClass(node, 'active'))
				return BX.PreventDefault(e);

			if(page == 'prev' || page == 'next') {
				pageNum = parseInt(BX.findParent(node).querySelector('.active').getAttribute('data-action'));
		...
		showDeliveryItemsPage: function(page) {
			...
			deliveryItemsContainer = this.deliveryBlockNode.querySelector('.bx-soa-pp-companies');
		...
		showPaySystemItemsPage: function(page) {
			...
			paySystemItemsContainer = this.paySystemBlockNode.querySelector('.bx-soa-pp-companies');
			...
			if(this.result.PAY_FROM_ACCOUNT == 'Y') {
				paySystemItemNode = this.getInnerPaySystem();
		...
		bindEvents: function() {
			...
			BX.addCustomEvent('OnOrderSaved', BX.delegate(function(orderNumber, redirectUrl) {
				BX.ajax({
					...
					onsuccess: function(result) {
						BX.onGlobalCustomEvent('OnOrderSavedBasketChange', [result]);
		...
		initOptions: function() {
			...
			if(this.result.GRID && this.result.GRID.ROWS) {
				var hasOldPrice = false,
					basketBasePrice = parseFloat(this.result.TOTAL.ORDER_PRICE) + parseFloat(this.result.TOTAL.DISCOUNT_PRICE);
				
				for(var j in this.result.GRID.ROWS) {
					if(this.result.GRID.ROWS.hasOwnProperty(j)) {
						var data = this.result.GRID.ROWS[j].data;
						if(data.PROPERTY_OLD_PRICE_VALUE && data.PROPERTY_OLD_PRICE_VALUE.length && data.DISCOUNT_PRICE_PERCENT == 0) {
							if(!hasOldPrice)
								hasOldPrice = true;							
							
							data.BASE_PRICE = data.PROPERTY_OLD_PRICE_VALUE;
							data.BASE_PRICE_FORMATED = BX.Currency.currencyFormat(data.BASE_PRICE, data.CURRENCY, true);							
							
							data.DISCOUNT_PRICE = (parseFloat(data.BASE_PRICE) - parseFloat(data.PRICE)).toString();
							data.DISCOUNT_PRICE_PERCENT = (Math.round(100 * parseFloat(data.DISCOUNT_PRICE) / parseFloat(data.BASE_PRICE))).toString();
							data.DISCOUNT_PRICE_PERCENT_FORMATED = data.DISCOUNT_PRICE_PERCENT + '%';
							
							data.SUM_BASE = (parseFloat(data.BASE_PRICE) * parseFloat(data.QUANTITY)).toString();
							data.SUM_BASE_FORMATED = BX.Currency.currencyFormat(data.SUM_BASE, data.CURRENCY, true);

							basketBasePrice += parseFloat(data.DISCOUNT_PRICE) * parseFloat(data.QUANTITY);
						} else if(data.CUSTOM_PRICE == 'Y') {
							data.DISCOUNT_PRICE = (parseFloat(data.BASE_PRICE) - parseFloat(data.PRICE)).toString();
							data.DISCOUNT_PRICE_PERCENT = (Math.round(100 * parseFloat(data.DISCOUNT_PRICE) / parseFloat(data.BASE_PRICE))).toString();
							data.DISCOUNT_PRICE_PERCENT_FORMATED = data.DISCOUNT_PRICE_PERCENT + '%';
						}
				...
				if(hasOldPrice) {
					this.result.TOTAL.PRICE_WITHOUT_DISCOUNT_VALUE = basketBasePrice.toString();
					this.result.TOTAL.PRICE_WITHOUT_DISCOUNT = BX.Currency.currencyFormat(this.result.TOTAL.PRICE_WITHOUT_DISCOUNT_VALUE, this.currency, true);

					this.result.TOTAL.DISCOUNT_PRICE = (parseFloat(this.result.TOTAL.PRICE_WITHOUT_DISCOUNT_VALUE) - parseFloat(this.result.TOTAL.ORDER_PRICE)).toString();
					this.result.TOTAL.DISCOUNT_PRICE_FORMATED = BX.Currency.currencyFormat(this.result.TOTAL.DISCOUNT_PRICE, this.currency, true);
				}
			}
		...
		clickNextAction: function(event) {
			...
			if((!this.result.IS_AUTHORIZED || typeof this.result.LAST_ORDER_DATA.FAIL !== 'undefined') && section.next.getAttribute('data-visited') == 'false') {
				...
				editStep = section.next.querySelector('.bx-soa-section-edit');
				if(editStep)
		...
		shouldSkipSection: function(section) {
			...
			if(this.params.SKIP_USELESS_BLOCK === 'Y') {
				...
				if(section.id === this.deliveryBlockNode.id) {
					skip = this.result.DELIVERY && this.result.DELIVERY.length === 1 && this.result.DELIVERY[0].EXTRA_SERVICES.length === 0 && !this.result.DELIVERY[0].CALCULATE_ERRORS;
				}
		...
		markSectionAsCompleted: function(section) {
			...
			if((!this.result.IS_AUTHORIZED || typeof this.result.LAST_ORDER_DATA.FAIL !== 'undefined') && section.getAttribute('data-visited') === 'false') {
			...
			if(!isLastNode) {
				buttons.push(
					BX.create('A', {
						props: {
							href: 'javascript:void(0)', 
							className: 'btn btn-buy pull-right'
						},
						html: '<span>' + this.params.MESS_FURTHER + '</span>',
						events: {
		...
		shouldBeSectionVisible: function(sections, currentPosition) {
			...
			for(; currentPosition < sections.length; currentPosition++) {
				...
				if(!this.firstLoad) {
					editStepNode = sections[currentPosition].querySelector('.bx-soa-section-edit');
		...
		changeVisibleContent: function() {
			...
			for(i = 0; i < sections.length; i++) {
				...
				if(this.firstLoad && skipFlag) {
					if(state && sections[i + 1] && this.checkBlockErrors(sections[i]) && ((orderDataLoaded && this.checkPreload(sections[i])) || (!orderDataLoaded && this.shouldSkipSection(sections[i])))) {
			...
			if((!this.result.IS_AUTHORIZED || typeof this.result.LAST_ORDER_DATA.FAIL !== 'undefined') && this.params.SHOW_ORDER_BUTTON === 'final_step') {
		...
		changeVisibleSection: function(section, state) {
			...
			editStep = section.querySelector('.bx-soa-section-edit');
			if(editStep)
		...
		editSection: function(section) {
			...
			if(this.result.SHOW_AUTH) {
				...
				BX.bind(titleNode, 'click', BX.proxy(this.showByClick, this));
				editButton = titleNode.querySelector('.bx-soa-section-edit');
				editButton && BX.bind(editButton, 'click', BX.proxy(this.showByClick, this));
			}

			errorContainer = section.querySelector('.alert.alert-error');
		...
		editAuthorizeForm: function(authContent) {
						...
						children: [
							BX.create('LABEL', {								
								children: [
									...
									BX.create('SPAN', {props: {className: 'check-title'}, text: BX.message('STOF_REMEMBER')})
								]
							})
						]
					})
			...
			button = BX.create('DIV', {
				...
				children: [
					...
					BX.create('BUTTON', {
						...
						html: '<span>' + BX.message('STOF_ENTER') + '</span>',
						events: {
			...
			authFormNode = BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {props: {className: 'h2'}, text: BX.message('STOF_AUTH_REQUEST')}),
					login,
					password,
					remember,
					button,
			...
			authContent.appendChild(authFormNode);
		},
		...
		createAuthFormInputContainer: function(labelText, inputNode, required) {
			...
			if(required)
				labelHtml += '<span class="bx-soa-starrequired">*</span>';

			return BX.create('DIV', {
				props: {className: 'form-group'},
				children: [
					BX.create('DIV', {props: {className: 'bx-soa-label'}, html: labelHtml}),
					inputNode
				]
			});
		},
		...
		editRegistrationForm: function(authContent) {
			...
			if(showSmsConfirm) {
				...
				authFormNodes.push(BX.create('DIV', {
					...
					children: [
						BX.create('BUTTON', {
							...
							html: '<span>' + BX.message('STOF_SEND') + '</span>',
							events: {
								click: BX.delegate(function(e) {
									...
									children: [
										BX.create('LABEL', {
											...
											children: [
												...
												BX.create('SPAN', {props: {className: 'check-title'}, text: BX.message('STOF_MY_PASSWORD')})
											],
											events: {
									...
									children: [
										BX.create('LABEL', {
											...
											children: [
												...
												BX.create('SPAN', {props: {className: 'check-title'}, text: BX.message('STOF_SYS_PASSWORD')})
											],
											events: {
												change: BX.delegate(function(){
													var generated = this.authBlockNode.querySelector('.generated');
				...
				if(this.result.AUTH.captcha_registration == 'Y') {
					authFormNodes.push(BX.create('DIV', {
						props: {className: 'form-group captcha'},
						children: [
							BX.create('DIV', {
								props: {className: 'bx-soa-label'},
								html: BX.message('CAPTCHA_REGF_PROMT') + ' <span class="bx-soa-starrequired">*</span>'
							}),
							BX.create('DIV', {
								props: {className: 'pic'},
								children: [
					...
					BX.create('DIV', {
						...
						children: [
							...
							BX.create('BUTTON', {
								...
								html: '<span>' + BX.message('STOF_REGISTER') + '</span>',
								events: {
									click: BX.delegate(function(e){
			...
			authContent.appendChild(				
				BX.create('DIV', {props: {className: 'bx-authform'}, children: authFormNodes})
			);
		...
		editSocialContent: function(authContent) {
			...
			if(this.socServiceHiddenNode) {
				authContent.appendChild(BX.create('DIV', {
					...
					html: '<div class="h2">' + BX.message('SOA_DO_SOC_SERV') + '</div>' + this.socServiceHiddenNode
				}));
			}
			...
			if(this.result.AUTH.new_user_registration === 'Y') {
				authContent.appendChild(BX.create('DIV', {
					...
					children: [
						...
						BX.create('A', {
							props: {className: 'btn btn-buy'},
							html: '<span>' + BX.message('STOF_DO_REGISTER') + '</span>',
							events: {
								click: BX.delegate(function(e) {
									this.toggleAuthForm(e);
									return BX.PreventDefault(e);
				...
				BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {
							...
							children: [
								BX.create('SPAN', {
									props: {className: 'bx-soa-section-title-icon'},
									children: [
										BX.create('I', {
											props: {
								...
								BX.create('SPAN', {
									props: {className: 'bx-soa-section-title-val'},
		...
		editActiveBasketBlock: function(activeNodeMode) {
			var node = !!activeNodeMode ? this.basketBlockNode : this.basketHiddenBlockNode,
				basketContent, basketTable, basketFooter;
			...
			if(this.initialized.basket) {
				...
				basketFooter = BX.create('DIV', {props: {className: 'bx-soa-item-footer'}});
				basketContent.appendChild(basketFooter);

				if(this.params.SHOW_COUPONS_BASKET === 'Y') {
					this.editCoupons(basketFooter);
				}

				this.getBlockFooter(basketFooter);

				BX.bind(
					basketContent.querySelector('.bx-soa-table-fade').firstChild,
					'scroll',
		...
		editFadeBasketBlock: function(activeNodeMode) {
			var node = !!activeNodeMode ? this.basketHiddenBlockNode : this.basketBlockNode,
				newContent, basketTable, basketFooter;
			...
			if(this.initialized.basket) {
				...
				if(this.params.SHOW_COUPONS_BASKET === 'Y') {
					this.editCouponsFade(basketFooter);
				}

				newContent.appendChild(basketFooter);

				node.appendChild(newContent);
				this.basketBlockScrollCheck();

				BX.bind(
					this.basketBlockNode.querySelector('.bx-soa-table-fade').firstChild,
					'scroll',
					BX.proxy(this.basketBlockScrollCheckEvent, this)
				);
			}
		...
		editBasketItemsHeader: function(basketItemsNode) {
			...
			var headers = [
					BX.create('DIV', {
						props: {className: 'bx-soa-item-td bx-soa-item-item'},
			...
			for(i = 0; i < this.result.GRID.HEADERS.length; i++) {
				...
				if(column.id === 'NAME' || column.id === 'PREVIEW_PICTURE' || column.id === 'PROPS' || column.id === 'NOTES' || column.id == 'PROPERTY_ARTNUMBER_VALUE' || column.id == 'PROPERTY_OBJECT_VALUE' || column.id == 'PROPERTY_OLD_PRICE_VALUE')
					continue;
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'hidden-xs hidden-sm bx-soa-item-tr'},
					children: headers
				})
			);
		},
		...
		createBasketItem: function(basketItemsNode, item, index, active) {
			...
			for(i = 0; i < this.result.GRID.HEADERS.length; i++) {
				...
				if(currentColumn.id === 'NAME' || currentColumn.id === 'PREVIEW_PICTURE' || currentColumn.id === 'PROPS' || currentColumn.id === 'NOTES' || currentColumn.id == 'PROPERTY_ARTNUMBER_VALUE' || currentColumn.id == 'PROPERTY_OBJECT_VALUE' || currentColumn.id == 'PROPERTY_OLD_PRICE_VALUE')
					continue;
			...
			if(hiddenColumns.length) {
				basketItemsNode.appendChild(
					BX.create('DIV', {
						props: {className: 'hidden-xs hidden-sm bx-soa-item-tr bx-soa-item-info-container'},
						children: [
							BX.create('DIV', {
								props: {className: 'bx-soa-item-td'},
								children: [
									BX.create('A', {
										props: {href: 'javascript:void(0)', className: 'bx-soa-info-shower'},
		...
		createBasketItemImg: function(data) {
			...
			logoNode = BX.create('DIV', {props: {className: 'bx-soa-item-image'}});
			...
			return BX.create('DIV', {
				props: {className: 'bx-soa-item-image-container'},
				children: [logoNode]
			});
		},

		createBasketItemContent: function(data) {
			var itemNode = BX.create('DIV', {props: {className: 'bx-soa-item-content'}}),
				column,
				itemName = data.NAME || '',
			...
			for(i = 0; i < this.result.GRID.HEADERS.length; i++) {
				column = this.result.GRID.HEADERS[i];
				if(column.id == 'PROPERTY_ARTNUMBER_VALUE') {
					itemNode.appendChild(BX.create('DIV', {
						props: {className: 'bx-soa-item-article'},
						html: column.name + ': ' + (data.PROPERTY_ARTNUMBER_VALUE && data.PROPERTY_ARTNUMBER_VALUE.length ? data.PROPERTY_ARTNUMBER_VALUE : '-')
					}));
				}
			}
			...
			if(this.options.showPropsInBasket && props.length) {
				for(var i in props) {
					if(props.hasOwnProperty(i)) {
						...
						if(code != 'ARTNUMBER') {
							itemNode.appendChild(BX.create('DIV', {
								props: {className: 'bx-soa-item-prop'},
								html: name + ': ' + value
							}));
						}
					}
		...
		createBasketItemColumn: function(column, allData, active) {
			...
			var data = allData.columns[column.id] ? allData.columns : allData.data,
				textNode = BX.create('DIV', {props: {className: 'hidden-xs hidden-sm bx-soa-item-td'}}),
				logotype = this.getImageSources(allData.data, 'DETAIL_PICTURE'),
				img;
			
			if(column.id === 'PRICE_FORMATED') {
				textNode.innerHTML = data.PRICE_FORMATED;
				if(parseFloat(data.DISCOUNT_PRICE) > 0) {
					textNode.appendChild(BX.create('DIV', {
						props: {className: 'bx-soa-item-old-price'},
				...
				if(this.options.showPriceNotesInBasket && active) {
					textNode.appendChild(BX.create('DIV', {
						props: {className: 'bx-soa-item-type-price'},
						text: data.NOTES
					}));
				}
			} else if(column.id === 'SUM') {
				BX.removeClass(textNode, 'hidden-xs hidden-sm');
				textNode.innerHTML = data.SUM;
				if(parseFloat(data.DISCOUNT_PRICE) > 0) {
					textNode.appendChild(BX.create('DIV', {
						props: {className: 'bx-soa-item-old-price'},
				...
				BX.addClass(textNode, 'bx-soa-item-discount');
				textNode.innerHTML = data.DISCOUNT_PRICE_PERCENT_FORMATED;
			} else if(column.id === 'DETAIL_PICTURE') {
				img = BX.create('IMG', {
					props: {
						className: 'bx-soa-item-image-popup',
				...
				if(column.id == 'DISCOUNT_PRICE_PERCENT_FORMATED')
					BX.addClass(textNode, 'bx-soa-item-discount-percent');
				if(column.id == 'QUANTITY') {
					BX.removeClass(textNode, 'hidden-xs hidden-sm');
					BX.addClass(textNode, 'bx-soa-item-quantity');
				}
				textNode.innerHTML = data[column.id];
			} else {
				...
				if(BX.type.isArray(columnData)) {
					for(var i in columnData) {
						if(columnData.hasOwnProperty(i)) {
							if(columnData[i].type == 'image') {
								...
								textNode.appendChild(BX.create('DIV', {html: columnData[i].value_format}));
							} else if(columnData[i].value) {
								textNode.appendChild(BX.create('DIV', {html: columnData[i].value}));
							}
						}
					}
					...
					if(val.length) {
						textNode.appendChild(
							BX.create('UL', {
								props: {className: 'bx-soa-item-image-list'},
								children: val
							})
						);
					}
				} else if(columnData) {
					textNode.appendChild(BX.create('DIV', {html: BX.util.htmlspecialchars(columnData)}));
				}
			}
		...
		createBasketItemHiddenColumn: function(column, allData) {
			...
			var data = allData.columns[column.id] ? allData.columns : allData.data,
				textNode = BX.create('DIV', {props: {className: 'bx-soa-info-text'}}),
				logotype = this.getImageSources(allData.data, 'DETAIL_PICTURE'),
				img, i;
			...
			if(column.id === 'PROPS') {
						...
						if(props.hasOwnProperty(i)) {
							...
							propsNodes.push(
								BX.create('DIV', {
									...
									children: [
										BX.create('DIV', {props: {className: 'bx-soa-info-title'}, text: name + ':'}),
										BX.create('DIV', {props: {className: 'bx-soa-info-text'}, html: BX.util.htmlspecialchars(value)})
									]
								})
							);
						}
				...
				textNode.appendChild(BX.create('DIV', {
					props: {className: 'bx-soa-item-discount'},
		...
		popupShow: function(e, url, source) {
			...
			this.popup = new BX.PopupWindow('bx-soa-image-popup', null, {
				...
				offsetTop: 0,
				offsetLeft: 0,
				closeIcon: {top: '-12px', right: '-12px'},
				closeByEsc: true,
				className: 'bx-soa-image-popup-window',				
				events: {
		...
		getImageContainer: function(link, source) {
			return BX.create('LI', {
				props: {className: 'bx-soa-item-image-item'},
				children: [
					BX.create('IMG', {
						attrs: {'src': link}
					})
		...
		editCoupons: function(basketItemsNode) {
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					...
					children: [
						couponsInput,
						BX.create('DIV', {props: {className: 'bx-soa-coupon-items'}, children: couponsList})
					]
				})
			);
		},
		...
		editCouponsFade: function(basketItemsNode) {
			...
			if(couponsList.length) {
				...
				couponsBlock = BX.create('DIV', {
					props: {className: 'bx-soa-coupon-items'},
					children: [couponsLabel].concat(couponsList)
				});
				
				basketItemsNode.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-coupon bx-soa-coupon-fixed'},
		...
		getCouponNode: function(coupon, active) {
			...
			return BX.create('DIV', {
				...
				children: active ? [				
					BX.create('SPAN', {
						props: {
							className: 'bx-soa-coupon-item-note ' + couponItem
						},
						html: couponName + ' ' + couponDesc
					}),
					BX.create('SPAN', {
						props: {className: 'bx-soa-coupon-item-remove-container'},
						children: [
							BX.create('SPAN', {
								props: {className: 'bx-soa-coupon-item-remove'},
								events: {
									click: BX.delegate(function(e){
										var target = e.target || e.srcElement,
											coupon = BX.findParent(target, {className: 'bx-soa-coupon-item'});
		...
		addCoupon: function(coupon) {
			var couponListNodes = this.orderBlockNode.querySelectorAll('.bx-soa-coupon:not(.bx-soa-coupon-fixed) .bx-soa-coupon-items'),
				couponInput = BX('coupon');

			for(var i = 0; i < couponListNodes.length; i++) {
				if(couponListNodes[i].querySelector('[data-coupon="' + BX.util.htmlspecialchars(coupon) + '"]'))
					break;

				couponListNodes[i].appendChild(this.getCouponNode({text: coupon}, true));
			}
		...
		editActiveRegionBlock: function(activeNodeMode) {
			var node = activeNodeMode ? this.regionBlockNode : this.regionHiddenBlockNode,
				regionContent, regionNode, regionFooter;
			...
			if(this.initialized.region) {
				...
				regionFooter = BX.create('DIV', {props: {className: 'bx-soa-item-footer'}});
				regionContent.appendChild(regionFooter);

				this.getBlockFooter(regionFooter);
			}
		},
		...
		editFadeRegionContent: function(node) {
			...
			var selectedPersonType = this.getSelectedPersonType(),
				errorNode = this.regionHiddenBlockNode.querySelector('.alert.alert-error'),
			...
			if(selectedPersonType && selectedPersonType.NAME && this.result.PERSON_TYPE.length > 1) {
				addedHtml += '<div class="form-group text"><div class="bx-soa-label">' + this.params.MESS_PERSON_TYPE + '</div>';
				addedHtml += '<div>' + BX.util.htmlspecialchars(selectedPersonType.NAME) + '</div></div>';
			}
			...
			for(i in this.result.ORDER_PROP.properties) {
				if(this.result.ORDER_PROP.properties.hasOwnProperty(i)) {
					if(this.result.ORDER_PROP.properties[i].IS_LOCATION == 'Y' && this.result.ORDER_PROP.properties[i].ID == this.deliveryLocationInfo.loc) {
						locationProperty = this.result.ORDER_PROP.properties[i];
					} else if(this.result.ORDER_PROP.properties[i].IS_ZIP == 'Y' && this.result.ORDER_PROP.properties[i].ID == this.deliveryLocationInfo.zip) {
			...
			if(locationProperty && locationString.length) {
				addedHtml += '<div class="form-group text"><div class="bx-soa-label">' + BX.util.htmlspecialchars(locationProperty.NAME) + '</div>';
				addedHtml += '<div>' + BX.util.htmlspecialchars(locationString) + '</div></div>';
			}

			if(zipProperty && zipValue.length) {
				addedHtml += '<div class="form-group text"><div class="bx-soa-label">' + BX.util.htmlspecialchars(zipProperty.NAME) + '</div>';
				addedHtml += '<div>' + BX.util.htmlspecialchars(zipValue) + '</div></div>';
			}
			...
			BX.bind(node.querySelector('.alert.alert-error'), 'click', BX.proxy(this.showByClick, this));
		...
		getDeliveryLocationInput: function(node) {
			...
			if(location && location[0] && location[0].output) {
				...
				labelHtml = '<div class="bx-soa-label">'					
					+ BX.util.htmlspecialchars(currentProperty.NAME)
					+ (currentProperty.REQUIRED == 'Y' ? '<span class="bx-soa-starrequired">*</span> ' : '')
			...
			if(altProperty) {
				...
				labelTextHtml += altProperty.REQUIRED == 'Y' ? '<span class="bx-soa-starrequired">*</span> ' : '';
		...
		getZipLocationInput: function(node) {
			...
			if(zipProperty) {
				...
				labelTextHtml += zipProperty.REQUIRED == 'Y' ? '<span class="bx-soa-starrequired">*</span> ' : '';
		...
		getPersonTypeControl: function(node) {
			...
			if(personTypesCount > 1) {
				input = BX.create('DIV', {
					props: {className: 'form-group'},
					children: [
						BX.create('DIV', {props: {className: 'bx-soa-label'}, html: this.params.MESS_PERSON_TYPE})
					]
				});
					...
					if(this.result.PERSON_TYPE.hasOwnProperty(i)) {
						...
						label = BX.create('LABEL', {
							children: [
								...
								BX.create('SPAN', {props: {className: 'check-title'}, text: BX.util.htmlspecialchars(currentType.NAME)})
							],
							events: {change: BX.proxy(this.sendRequest, this)}
						});

						node.appendChild(BX.create('DIV', {props: {className: 'radio'}, children: [label]}));
		...
		getProfilesControl: function(node) {
			...
			if(profilesLength) {
				if(this.params.ALLOW_USER_PROFILES === 'Y' && (profilesLength > 1 || this.params.ALLOW_NEW_PROFILE === 'Y')) {
					...
					label = BX.create('DIV', {props: {className: 'bx-soa-label'}, html: this.params.MESS_SELECT_PROFILE});
		...
		editActivePaySystemBlock: function(activeNodeMode) {
			var node = activeNodeMode ? this.paySystemBlockNode : this.paySystemHiddenBlockNode,
				paySystemContent, paySystemNode, paySystemFooter;
			...
			if(this.initialized.paySystem) {
				...
				paySystemFooter = BX.create('DIV', {props: {className: 'bx-soa-item-footer'}});
				paySystemContent.appendChild(paySystemFooter);

				if(this.params.SHOW_COUPONS_PAY_SYSTEM == 'Y')
					this.editCoupons(paySystemFooter);

				this.getBlockFooter(paySystemFooter);
			}
		},
		...
		editPaySystemItems: function(paySystemNode) {
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-companies'}}),
				paySystemItemNode, i;
		...
		createPaySystemItem: function(item) {
			var checked = item.CHECKED == 'Y',
				...
				label, price, info, inject, itemNode;
			...
			label = BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {
						props: {className: 'bx-soa-pp-company-checkbox'},
						children: [
							BX.create('INPUT', {
								props: {
			...
			if(item.PRICE && parseFloat(item.PRICE) > 0) {
				price = BX.create('DIV', {
					props: {
						className: 'bx-soa-pp-company-cost-container'
					},
					children: [
						BX.create('DIV', {
							props: {className: 'bx-soa-pp-company-cost'},
			...
			info = BX.create('DIV', {props: {className: 'bx-soa-pp-company-info'}});
									
			if(this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y') {
				info.appendChild(BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-title'},
					text: item.NAME
				}));
			}

			inject = BX.create('DIV', {props: {className: 'bx-soa-pp-company-inject' + (!checked ? ' hidden' : '')}});
			if(checked)
				inject.appendChild(BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc'}}));
			info.appendChild(inject);
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-container'},
				children: [
					BX.create('DIV', {
						props: {
							className: 'bx-soa-pp-company' + (item.PRICE && parseFloat(item.PRICE) > 0 ? ' bx-has-price' : '') + (checked ? ' bx-selected' : '')
						},
						children: [label, info, price],
						events: {
		...
		getInnerPaySystem: function() {
			...
			var accountOnly = this.params.ONLY_FULL_PAY_FROM_ACCOUNT && (this.params.ONLY_FULL_PAY_FROM_ACCOUNT == 'Y'),
				...
				logotype, logoNode,
				label, price, info, itemNode;

			logoNode = BX.create('DIV', {props: {className: 'bx-soa-pp-company-image'}});
			logotype = this.getImageSources(paySystem, 'PSA_LOGOTIP');
			if(logotype && logotype.src_2x) {
				logoNode.setAttribute('style',
					'background-image: url("' + logotype.src_1x + '");' +
					'background-image: -webkit-image-set(url("' + logotype.src_1x + '") 1x, url("' + logotype.src_2x + '") 2x)'
			...
			price = BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {
						props: {className: 'bx-soa-pp-company-cost'},
						html: this.result.CURRENT_BUDGET_FORMATED
					})
				]
			});
			...
			itemNode = BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {
						props: {
							className: 'bx-soa-pp-company bx-soa-pp-inner-ps bx-has-price' + (isSelected ? ' bx-selected' : '')
						},
						children: [label, info, price],
						events: {
							click: BX.proxy(this.selectPaySystem, this)
		...
		editFadePaySystemContent: function(node) {
			var selectedPaySystem = this.getSelectedPaySystem(),
				errorNode = this.paySystemHiddenBlockNode.querySelector('div.alert.alert-error'),
			...
			if(this.isSelectedInnerPayment()) {
				...
				addedHtml += '<div class="bx-soa-pp-company-selected-image"><img src="' + imgSrc + '" alt="' + this.result.INNER_PAY_SYSTEM.NAME + '"/></div>';
				addedHtml += '<div class="bx-soa-pp-company-selected-title">' + this.result.INNER_PAY_SYSTEM.NAME + '</div>';
				addedHtml += '</div>';
			}
			...
			if(selectedPaySystem && selectedPaySystem.NAME) {
				...
				addedHtml += '<div class="bx-soa-pp-company-selected">';
				addedHtml += '<div class="bx-soa-pp-company-selected-image"><img src="' + imgSrc + '" alt="' + BX.util.htmlspecialchars(selectedPaySystem.NAME) + '"/></div>';
				addedHtml += '<div class="bx-soa-pp-company-selected-title">' + BX.util.htmlspecialchars(selectedPaySystem.NAME) + '</div>';
				addedHtml += '</div>';
			}

			if(!addedHtml.length)
				addedHtml = '<div>' + BX.message('SOA_PS_SELECT_ERROR') + '</div>';
		...
		editActiveDeliveryBlock: function(activeNodeMode) {
			var node = activeNodeMode ? this.deliveryBlockNode : this.deliveryHiddenBlockNode,
				deliveryContent, deliveryNode, deliveryFooter;
			...
			if(this.initialized.delivery) {
				...
				deliveryFooter = BX.create('DIV', {props: {className: 'bx-soa-item-footer'}});
				deliveryContent.appendChild(deliveryFooter);

				if(this.params.SHOW_COUPONS_DELIVERY == 'Y')
					this.editCoupons(deliveryFooter);

				this.getBlockFooter(deliveryFooter);
			}
		},
		...
		editDeliveryItems: function(deliveryNode) {
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-companies'}}),
				deliveryItemNode, k;
		...
		getDeliveryPriceNodes: function(delivery) {
			...
			if(typeof delivery.DELIVERY_DISCOUNT_PRICE !== 'undefined' && parseFloat(delivery.DELIVERY_DISCOUNT_PRICE) != parseFloat(delivery.PRICE)) {
		...
		getDeliveryExtraServices: function(delivery) {
			...
			for(i in delivery.EXTRA_SERVICES) {
				...
				if(currentService.editControl.indexOf('this.checked') == -1) {
					serviceName = BX.create('DIV', {
						props: {
							className: 'bx-soa-label'
						},
						...
						+ (currentService.description && currentService.description.length
							? ' <small>(' + BX.util.htmlspecialchars(currentService.description) + ')</small>'
							: '')
					});
		...
		createDeliveryItem: function(item) {
			var checked = item.CHECKED == 'Y',
				...
				logotype, label, price, info, inject, extraServices, extraServicesNode, itemNode, logoNode;
			...
			price = BX.create('DIV', {props: {className: 'bx-soa-pp-company-cost-container'}});
			if(item.PRICE >= 0) {
				price.appendChild(BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-cost'},
					children: this.getDeliveryPriceNodes(item)
				}));
			} else if(deliveryCached && deliveryCached.PRICE >= 0) {
				price.appendChild(BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-cost'},
					children: this.getDeliveryPriceNodes(deliveryCached)
				}));
			}
			...
			if(item.PERIOD_TEXT && item.PERIOD_TEXT.length) {
				info.appendChild(BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-period'},
					html: item.PERIOD_TEXT
				}));
			} else if(deliveryCached && deliveryCached.PERIOD_TEXT && deliveryCached.PERIOD_TEXT.length) {
				info.appendChild(BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-period'},
					html: deliveryCached.PERIOD_TEXT
				}));
			}
			...
			if(item.CALCULATE_DESCRIPTION) {
				info.appendChild(BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-description'},
					html: item.CALCULATE_DESCRIPTION
				}));
			}
			
			if(checked) {
				extraServices = this.getDeliveryExtraServices(item);
				if(extraServices.length) {
					extraServicesNode = BX.create('DIV', {
						props: {className: 'bx-soa-pp-company-services'},
						children: extraServices
					});
					info.appendChild(extraServicesNode);
				}
			}
			...
			itemNode = BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {
						props: {
							className: 'bx-soa-pp-company' + (item.PRICE >= 0 || (deliveryCached && deliveryCached.PRICE >= 0) ? ' bx-has-price' : '') + (checked ? ' bx-selected' : '')
						},
						children: [label, info, price],
						events: {
							click: BX.proxy(this.selectDelivery, this)
						}
					})
				]
			});
			...
			if(checked && this.params.DELIVERY_NO_AJAX != 'Y')
				this.deliveryCachedInfo[deliveryId] = item;
		...
		editFadeDeliveryContent: function(node) {
			var selectedDelivery = this.getSelectedDelivery(),
				...
				errorNode = this.deliveryHiddenBlockNode.querySelector('div.alert.alert-error'),
			...
			if(selectedDelivery && selectedDelivery.NAME) {
				...
				node.appendChild(BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {
							props: {className: 'bx-soa-pp-company-selected-image'},
							children: [
								BX.create('IMG', {
									props: {
										src: imgSrc,
						...
						BX.create('DIV', {
							props: {className: 'bx-soa-pp-company-selected-title'},
							text: name
						}),
						BX.create('DIV', {
							props: {className: 'bx-soa-pp-company-selected-price'},
							...
							if(extraService.value && extraService.value != 'N' && extraService.canUserEditValue) {
								node.appendChild(BX.create('DIV', {
									...
									children: [
										BX.create('DIV', {
											props: {className: 'bx-soa-pp-company-selected-title'},
											text: extraService.name
										}),
										BX.create('DIV', {
											props: {className: 'bx-soa-pp-company-selected-price'},
											text: extraService.viewControl
										})
									]
								}));
							}
				...
				node.appendChild(BX.create('DIV', {text: BX.message('SOA_DELIVERY_SELECT_ERROR')}));
			}
		...
		selectDelivery: function(event) {			
			...
			var target = event.target || event.srcElement,
				actionCheckbox = BX.hasClass(target, 'checkbox') ? target : BX.findParent(target, {className: 'checkbox'}),
				actionSection =  BX.hasClass(target, 'bx-soa-pp-company') ? target : BX.findParent(target, {className: 'bx-soa-pp-company'}),
				selectedSection = this.deliveryBlockNode.querySelector('.bx-soa-pp-company.bx-selected'),
				actionCheckboxInput, actionSectionInput, selectedSectionInput;

			if(actionCheckbox) {
				actionCheckboxInput = actionCheckbox.querySelector('input[type=checkbox]');
				if(actionCheckboxInput.checked) {
					actionCheckboxInput.checked = false;
				} else {
					actionCheckboxInput.checked = true;
				}
				$(actionCheckboxInput).change();
			} else if(actionSection) {			
				if(BX.hasClass(actionSection, 'bx-selected'))
					return BX.PreventDefault(event);
				
				actionSectionInput = actionSection.querySelector('input[type=checkbox]');
				BX.addClass(actionSection, 'bx-selected');
				actionSectionInput.checked = true;
			
				if(selectedSection) {
					selectedSectionInput = selectedSection.querySelector('input[type=checkbox]');
					BX.removeClass(selectedSection, 'bx-selected');
					selectedSectionInput.checked = false;
				}
		...
		activatePickUp: function(deliveryName) {
			...
			this.pickUpBlockNode.querySelector('.bx-soa-section-title-val').innerHTML = BX.util.htmlspecialchars(deliveryName);
		...
		editActivePickUpBlock: function(activeNodeMode) {
			var node = activeNodeMode ? this.pickUpBlockNode : this.pickUpHiddenBlockNode,
				pickUpContent, pickUpContentContainer, pickUpFooter;
			...
			if(this.initialized.pickup) {
				...
				pickUpContentContainer = BX.create('DIV', {props: {className: 'bx_soa_pickup'}});
				
				this.editPickUpMap(pickUpContentContainer);
				this.editPickUpLoader(pickUpContentContainer);

				pickUpContent.appendChild(pickUpContentContainer);
				...
				pickUpFooter = BX.create('DIV', {props: {className: 'bx-soa-item-footer'}});
				pickUpContent.appendChild(pickUpFooter);

				this.getBlockFooter(pickUpFooter);
			}
		},
		...
		editFadePickUpContent: function(pickUpContainer) {
			...
			if(selectedPickUp) {
				html += '<div class="bx-soa-pp-company-selected">';
				...
				if(this.params.SHOW_STORES_IMAGES == 'Y') {
					...
					html += '<div class="bx-soa-pp-company-selected-image">';
					html += '<img src="' + imgSrc + '" class="bx-soa-pickup-preview-img" alt="' + BX.util.htmlspecialchars(selectedPickUp.TITLE) + '"/>';
					html += '</div>';
				}

				html += '<div class="bx-soa-pp-company-selected-title">' + BX.util.htmlspecialchars(selectedPickUp.TITLE);
				if(selectedPickUp.ADDRESS)
					html += '<br /><span>' + BX.message('SOA_PICKUP_ADDRESS') + ': ' + BX.util.htmlspecialchars(selectedPickUp.ADDRESS) + '</span>';

				if(selectedPickUp.PHONE)
					html += '<br /><span>' + BX.message('SOA_PICKUP_PHONE') + ': ' + BX.util.htmlspecialchars(selectedPickUp.PHONE) + '</span>';

				if(selectedPickUp.SCHEDULE)
					html += '<br /><span>' + BX.message('SOA_PICKUP_WORK') + ': ' + BX.util.htmlspecialchars(selectedPickUp.SCHEDULE) + '</span>';

				if(selectedPickUp.DESCRIPTION)
					html += '<br /><span>' + BX.message('SOA_PICKUP_DESC') + ': ' + BX.util.htmlspecialchars(selectedPickUp.DESCRIPTION) + '</span>';
		...
		editPickUpList: function(isNew) {
			...
			if(!!isNew) {
				container = this.pickUpHiddenBlockNode.querySelector('.bx_soa_pickup');
				if(!container)
					container = this.pickUpBlockNode.querySelector('.bx_soa_pickup');
		...
		createPickUpItem: function(currentStore, options) {
			...
			if(this.params.SHOW_STORES_IMAGES === 'Y') {
				...
				logoNode = BX.create('DIV', {
					props: {className: 'bx-soa-pickup-l-item-img-container'},
					children: [
						BX.create('IMG', {
							props: {
								src: imgSrc,
				...
				children: [
					...
					BX.create('DIV', {
						...
						children: [
							logoNode,
							BX.create('DIV', {
								props: {className: 'bx-soa-pickup-l-item-info'},
								children: [
					...
					BX.create('DIV', {
						...
						children: [
							BX.create('A', {
								...
								html: '<span>' + this.params.MESS_SELECT_PICKUP + '</span>',
								events: {
		...
		editActivePropsBlock: function(activeNodeMode) {
			var node = activeNodeMode ? this.propsBlockNode : this.propsHiddenBlockNode,
				propsContent, selectedDelivery, showPropMap = false, i, validationErrors;
		...
		editFadePropsContent: function(node) {
			...
			if(!props || props.length === 0) {
				node.innerHTML += '<div class="bx-soa-label">' + BX.message('SOA_ORDER_PROPS') + '</strong>';
			} else {
		...
		editPropsItems: function(propsNode) {
			...
			while(group = groupIterator()) {
				propsIterator =  group.getIterator();
				while(property = propsIterator()) {
					if(this.deliveryLocationInfo.loc == property.getId() || this.deliveryLocationInfo.zip == property.getId() || this.deliveryLocationInfo.city == property.getId())
						continue;
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled) {
			var propsItemNode = BX.create('DIV', {props: {className: 'form-group'}}),
				textHtml = '',
			...
			if(disabled) {
				BX.addClass(propsItemNode, "text");
				propsItemNode.innerHTML = '<div class="bx-soa-label">' + BX.util.htmlspecialchars(property.getName()) + '</div>';
			} else {
				...
				if(property.isRequired())
					textHtml += '<span class="bx-soa-starrequired">*</span> ';
				...
				label = BX.create('DIV', {
					attrs: {'data-id': 'soa-property-' + property.getId()},
					props: {className: 'bx-soa-label'},
					html: textHtml
				});
		...
		insertLocationProperty: function(property, propsItemNode, disabled) {
			...
			if(property.getId() in this.locations) {
				if(disabled) {
					...
					propsItemNode.innerHTML += '<div>' + values.join('<br />') + '</div>';
				} else {
					...
					for(i = 0; i < propRow.length; i ++) {
						...
						if(property.isMultiple())
							insertedLoc.style.marginBottom = '12px';
					...
					if(property.isMultiple()) {
						propContainer.appendChild(
							BX.create('DIV', {
								...
								props: {className: 'btn btn-buy'},
								html: '<span>' + BX.message('ADD_DEFAULT') + '</span>',
								events: {
		...
		insertDateProperty: function(property, propsItemNode, disabled) {
			...
			if(disabled) {
				...
				if(prop) {
					...
					propsItemNode.innerHTML += '<div>' + this.valuesToString(values) + '</div>';
				}
			} else {
				propContainer = BX.create('DIV', {props: {className: 'soa-property-container'}});
					...
					if(inputs.length) {
						for(i = 0; i < inputs.length; i++) {
							if(inputs[i].value.length) {
								if(property.getSettings()['IS_PHONE'] == 'Y' && property.getSettings()['MULTIPLE'] != 'Y' && !!this.params.PHONE_MASK) {
									if(this.iti[property.getId()].getNumber() != '')
										values.push(this.iti[property.getId()].getNumber().replace('+', ''));
									else if(property.getSettings()['VALUE'] != '')
										values.push(property.getSettings()['VALUE']);
									else
										values.push(this.iti[property.getId()].getSelectedCountryData().dialCode + inputs[i].value.replace(' ', '').replace(/\-/g, ''));
								} else {
		...
		insertYNProperty: function(property, propsItemNode, disabled) {
			var prop, inputs, values, i, propContainer, label;
			...
			if(disabled) {
				...
				propContainer = BX.create('DIV', {props: {className: 'soa-property-container checkbox'}});
				label = BX.create('LABEL');
				property.appendTo(label);				
		...
		alterProperty: function(settings, propContainer) {
			...
			if(divs && divs.length) {
				for(i = 0; i < divs.length; i++) {
					divs[i].style.marginBottom = '12px';
				}
			}
			...
			if(textNode) {
				...
				if(settings.IS_PHONE == 'Y' && settings.MULTIPLE != 'Y' && !!this.params.PHONE_MASK) {
					if(settings.VALUE != '') {
						textNode.value = '+' + settings.VALUE;
					} else if(BX.getCookie(BX.message('COOKIE_NAME') + '_ENEXT_FORMS_PHONE')) {
						textNode.value = decodeURIComponent(BX.getCookie(BX.message('COOKIE_NAME') + '_ENEXT_FORMS_PHONE').replace(/\+/g, ' '));
					}

					this.iti[settings.ID] = window.intlTelInput(textNode, {
						autoHideDialCode: false,
						autoPlaceholder : 'aggressive',					
						customPlaceholder: function(selectedCountryPlaceholder, selectedCountryData) {
							return selectedCountryPlaceholder.replace(/[0-9]/g, '_');
						},
						initialCountry: BX.getCookie(BX.message('COOKIE_NAME') + '_ENEXT_FORMS_COUNTRY') ? BX.getCookie(BX.message('COOKIE_NAME') + '_ENEXT_FORMS_COUNTRY') : (this.initialCountry != '' ? this.initialCountry : this.params.INITIAL_COUNTRY),
						nationalMode: false,
					...
					this.iti[settings.ID].promise.then(BX.delegate(function() {
						if(textNode.placeholder == '')
							this.iti[settings.ID]._updatePlaceholder();
						
						Inputmask(textNode.placeholder.replace(/_/g, '9')).mask(textNode);
					}, this));
					
					textNode.addEventListener('countrychange', BX.delegate(function(e) {
			...
			for(i = 0; i < inputs.length; i++) {
				if(settings.IS_PHONE != 'Y' || (settings.IS_PHONE == 'Y' && !this.params.PHONE_MASK)) {
			...
			if(settings.TYPE == 'FILE') {
				...
				for(i = 0; i < fileTitles.length; i++) {
					BX.addClass(fileTitles[i], 'bx-soa-file');
			...
			for(i = 0; i < add.length; i++) {
				...
				if(settings.TYPE == 'FILE') {
					...
					BX.adjust(add[i], {props: {className: 'btn btn-default'}});
				}
			}
			...
			if(add.length) {
				...
				BX.bind(add, 'click', BX.delegate(function(e) {
					...
					if(settings.TYPE == 'FILE') {
						...
						BX.addClass(fileTitles[fileTitles.length - 1], 'bx-soa-file');
		...
		isValidForm: function() {
			...
			if(propsErrors.length && !navigated) {
				if(this.activeSectionId == this.propsBlockNode.id) {
					tooltips = this.propsBlockNode.querySelectorAll('div.bx-soa-tooltip');
			...
			if(!tooltip) {
				tooltip = BX.create('DIV', {
					props: {
						...
						className: 'bx-soa-tooltip alert alert-error'
					}
				});
		...
		showValidationResult: function(inputs, errors) {
			...
			var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
				...
				label = formGroup.querySelector('.bx-soa-label'),
				tooltipId, inputDiv, i;

			if(label)
				tooltipId = label.getAttribute('data-id');
		...
		validateString: function(input, arProperty, fieldName) {
			...
			if(value.length) {
				...
				if(arProperty.IS_PHONE === 'Y' && !!this.params.PHONE_MASK) {
					if(this.iti[arProperty.ID].getNumber() != '' && !this.iti[arProperty.ID].isValidNumber())
		...
		validateFile: function(inputs, arProperty, fieldName) {
			...
			if(arProperty.REQUIRED == 'Y' && files.length == 0 && defaultValue == '' && (!arProperty.DEFAULT_VALUE || !arProperty.DEFAULT_VALUE.length)) {
				errors.push(field + ' ' + BX.message('SOA_REQUIRED'));
			} else {
		...
		editPropsMap: function(propsNode) {
			var propsMapContainer = BX.create('DIV', {props: {className: 'form-group'}, style: {width: '100%'}}),
		...
		editPropsComment: function(propsNode) {
			...
			label = BX.create('DIV', {
				attrs: {'data-id': 'orderDescription'},
				props: {className: 'bx-soa-label'},
		...
		editTotalBlock: function() {
			...
			if(this.options.showPriceWithoutDiscount) {
				priceHtml += '<span class="bx-price-old">' + total.PRICE_WITHOUT_DISCOUNT + '</span>';
			}

			if(this.options.showDiscountPrice) {
				priceHtml += '<span class="bx-price-discount">' + this.params.MESS_ECONOMY + ' ' + total.DISCOUNT_PRICE_FORMATED + '</span>';
			}
			...
			if(deliveryError) {
				...
				if(curDelivery && typeof curDelivery.DELIVERY_DISCOUNT_PRICE !== 'undefined' && parseFloat(curDelivery.PRICE) > parseFloat(curDelivery.DELIVERY_DISCOUNT_PRICE)) {
					deliveryValue += '<span class="bx-price-old">' + curDelivery.PRICE_FORMATED + '</span>';
				}
			}
			...
			if(!this.result.SHOW_AUTH) {
				this.totalInfoBlockNode.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-cart-total-button-container' + (!showOrderButton ? ' visible-xs visible-sm' : '')},
						children: [
							BX.create('A', {
								props: {
									href: 'javascript:void(0)',
									className: 'btn btn-buy btn-order-save'
								},
								html: '<span>' + this.params.MESS_ORDER + '</span>',
								events: {
		...
		editMobileTotalBlock: function() {
			if(this.result.SHOW_AUTH)
				BX.removeClass(this.mobileTotalBlockNode, 'visible-xs visible-sm');
			else
				BX.addClass(this.mobileTotalBlockNode, 'visible-xs visible-sm');
		...
		createTotalUnit: function(name, value, params) {
			...
			if(params.error) {
				totalValue = [BX.create('A', {
					props: {
						className: 'bx-soa-price-not-calc',
						href: 'javascript:void(0)'
					},
					html: value,
					events: {
		...
		totalBlockScrollCheck: function() {
			...
			if((scrollTop + topPanelHeight > ghostTop) && !BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed')) {
			...
			} else if((scrollTop + topPanelHeight < ghostTop) && BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed')) {
