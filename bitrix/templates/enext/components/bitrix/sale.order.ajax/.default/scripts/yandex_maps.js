...
(function() {
	...
	BX.Sale.OrderAjaxComponent.Maps = {
		...
		initializePickUpMap: function(selected) {
			...
			this.pickUpMap.behaviors.enable('drag');
			this.pickUpMap.behaviors.enable('dblClickZoom');
			this.pickUpMap.behaviors.enable('multiTouch');
			this.pickUpMap.behaviors.enable('rightMouseButtonMagnifier');

			this.pickUpMap.behaviors.disable('scrollZoom');
			this.pickUpMap.behaviors.disable('leftMouseButtonMagnifier');
			this.pickUpMap.behaviors.disable('ruler');
			this.pickUpMap.behaviors.disable('routeEditor');

			this.pickUpMap.controls.add('fullscreenControl');
			this.pickUpMap.controls.add('zoomControl');

			this.pickUpMap.controls.remove('geolocationControl');
			this.pickUpMap.controls.remove('routeEditor');
			this.pickUpMap.controls.remove('rulerControl');
			this.pickUpMap.controls.remove('searchControl');
			this.pickUpMap.controls.remove('trafficControl');
			this.pickUpMap.controls.remove('typeSelector');
		...
		buildBalloons: function(activeStores) {
			...
			for(var i = 0; i < activeStores.length; i++) {
				var logotype = that.context.getImageSources(activeStores[i], 'IMAGE_ID'),
				...
				var geoObj = new ymaps.Placemark([activeStores[i].GPS_N, activeStores[i].GPS_S], {
					...
					balloonContentLayout: ymaps.templateLayoutFactory.createClass(
						'<div class="pickup-item-marker pickup-item-marker-ymaps">'
						+ '<div class="pickup-item-marker-image"><img src="' + imgSrc + '" /></div>'
						+ '<div class="pickup-item-marker-caption"><div class="pickup-item-marker-title">{{ properties.storeTitle }}</div>'
						+ '{{ properties.storeBody|raw }}'
						+ '<a class="pickup-item-marker-link" href="javascript:void(0)" data-store="{{ properties.id }}">{{ properties.text }}</a></div>',
						{
							build: function() {
				...
				if(BX('BUYER_STORE').value === activeStores[i].ID) {
					geoObj.options.set('preset', 'islands#darkGreenDotIcon');
				}
			...
			if(this.pickUpMap && this.pickUpMap.geoObjects) {
				this.pickUpMap.geoObjects.each(BX.delegate(function(placeMark) {
					...
					if(placeMark.properties.get('id') === storeItemId) {
						placeMark.options.set({preset: 'islands#darkGreenDotIcon'});
			...
			if(this.pickUpMap && this.pickUpMap.geoObjects) {
				...
				this.pickUpMap.geoObjects.each(function(geoObject){
					if(geoObject.properties.get('id') === buyerStoreInput.value) {
						geoObject.options.set({preset: 'islands#darkGreenDotIcon'});
					} else if(parseInt(geoObject.properties.get('id')) > 0) {
						geoObject.options.set('preset', 'islands#violetIcon');
					}
				});
			}
		},
		...
		initializePropsMap: function(propsMapData) {
			...
			this.propsMap.behaviors.enable('drag');
			this.propsMap.behaviors.enable('dblClickZoom');
			this.propsMap.behaviors.enable('multiTouch');
			this.propsMap.behaviors.enable('rightMouseButtonMagnifier');

			this.propsMap.behaviors.disable('scrollZoom');
			this.propsMap.behaviors.disable('leftMouseButtonMagnifier');
			this.propsMap.behaviors.disable('ruler');
			this.propsMap.behaviors.disable('routeEditor');

			this.propsMap.controls.add('fullscreenControl');
			this.propsMap.controls.add('zoomControl');

			this.propsMap.controls.remove('geolocationControl');
			this.propsMap.controls.remove('routeEditor');
			this.propsMap.controls.remove('rulerControl');
			this.propsMap.controls.remove('searchControl');
			this.propsMap.controls.remove('trafficControl');
			this.propsMap.controls.remove('typeSelector');
