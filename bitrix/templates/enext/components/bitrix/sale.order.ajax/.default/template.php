...
if(!empty($arResult['GRID']['ROWS'])) {
	foreach($arResult['GRID']['ROWS'] as $arItem) {
		if(!empty($arItem['data']['PROPERTY_OBJECT_VALUE'])) {
			$objectId = $arItem['data']['PROPERTY_OBJECT_VALUE'];
			break;
		}
	}
	unset($arItem);
...
if($siteId != SITE_ID && $objectId)
	CJSCore::Init(array("ls"));

global $arSettings;
$arParams['MIN_ORDER_SUM'] = intval($arSettings['MIN_ORDER_SUM']['VALUE']);
$arParams['INITIAL_COUNTRY'] = strtolower($arSettings['FORMS_DEFAULT_COUNTRY']['VALUE']);
$arParams['PHONE_MASK'] = $arSettings['FORMS_PHONE_MASK']['VALUE'] == 'Y' ? true : false;
...
} elseif($siteId == SITE_ID && !$objectId && $arParams['MIN_ORDER_SUM'] > 0 && $arResult['ORDER_PRICE'] < $arParams['MIN_ORDER_SUM']) {
	if($arParams['DISABLE_BASKET_REDIRECT'] === 'Y')
		include(Main\Application::getDocumentRoot().$templateFolder.'/min_order_sum.php');
	else
		LocalRedirect($arParams['PATH_TO_BASKET']);
} elseif($siteId == SITE_ID && $objectId) {
	LocalRedirect($arParams['PATH_TO_BASKET']);
} else {
			...
			<div class="col-md-9 bx-soa">
				...
				<div id="bx-soa-auth" class="bx-soa-section bx-soa-auth" style="display: none;">
					<div class="bx-soa-section-title-container">
						<div class="bx-soa-section-title">
							<span class="bx-soa-section-title-icon"><i class="icon-login"></i><i class="icon-ok-circle"></i><i class="icon-danger"></i><i class="icon-exclamation-circle"></i></span>
							<span class="bx-soa-section-title-val"><?=$arParams['MESS_AUTH_BLOCK_NAME']?></span>
						</div>
					</div>
					<div class="bx-soa-section-content"></div>
				</div>
				<?//DUPLICATE_MOBILE_ORDER_SAVE_BLOCK//?>
				<div id="bx-soa-total-mobile" style="margin-bottom: 18px;"></div>
				...
				<?if(!isset($arParams['BASKET_POSITION']) || $arParams['BASKET_POSITION'] === 'before') {
					...
					<div id="bx-soa-basket" data-visited="false" class="bx-soa-section bx-active">
						<div class="bx-soa-section-title-container">
							<div class="bx-soa-section-title">
								<span class="bx-soa-section-title-icon"><i class="icon-bag-shop"></i><i class="icon-ok-circle"></i><i class="icon-danger"></i><i class="icon-exclamation-circle"></i></span>
								<span class="bx-soa-section-title-val"><?=$arParams['MESS_BASKET_BLOCK_NAME']?></span>
							</div>
				...
				<div id="bx-soa-region" data-visited="false" class="bx-soa-section bx-active">
					<div class="bx-soa-section-title-container">
						<div class="bx-soa-section-title">
							<span class="bx-soa-section-title-icon"><i class="icon-map-marker"></i><i class="icon-ok-circle"></i><i class="icon-danger"></i><i class="icon-exclamation-circle"></i></span>
							<span class="bx-soa-section-title-val"><?=$arParams['MESS_REGION_BLOCK_NAME']?></span>
						</div>
						<div class="bx-soa-section-edit">
							<a href="javascript:void(0)" class="bx-soa-editstep"><?=$arParams['MESS_EDIT']?></a>
						</div>
				...
				<?if($arParams['DELIVERY_TO_PAYSYSTEM'] === 'p2d') {
					...
					<div id="bx-soa-paysystem" data-visited="false" class="bx-soa-section bx-active">
						<div class="bx-soa-section-title-container">
							<div class="bx-soa-section-title">
								<span class="bx-soa-section-title-icon"><i class="icon-card"></i><i class="icon-ok-circle"></i><i class="icon-danger"></i><i class="icon-exclamation-circle"></i></span>
								<span class="bx-soa-section-title-val"><?=$arParams['MESS_PAYMENT_BLOCK_NAME']?></span>
							</div>
							<div class="bx-soa-section-edit">
								<a href="javascript:void(0)" class="bx-soa-editstep"><?=$arParams['MESS_EDIT']?></a>
							</div>
					...
					<div id="bx-soa-delivery" data-visited="false" class="bx-soa-section bx-active" <?=($hideDelivery ? 'style="display: none;"' : '')?>>
						<div class="bx-soa-section-title-container">
							<div class="bx-soa-section-title">
								<span class="bx-soa-section-title-icon"><i class="icon-delivery"></i><i class="icon-ok-circle"></i><i class="icon-danger"></i><i class="icon-exclamation-circle"></i></span>
								<span class="bx-soa-section-title-val"><?=$arParams['MESS_DELIVERY_BLOCK_NAME']?></span>
							</div>
							<div class="bx-soa-section-edit">
								<a href="javascript:void(0)" class="bx-soa-editstep"><?=$arParams['MESS_EDIT']?></a>
							</div>
					...
					<div id="bx-soa-pickup" data-visited="false" class="bx-soa-section" style="display:none">
						<div class="bx-soa-section-title-container">
							<div class="bx-soa-section-title">
								<span class="bx-soa-section-title-icon"><i class="icon-delivery"></i><i class="icon-ok-circle"></i><i class="icon-danger"></i><i class="icon-exclamation-circle"></i></span>
								<span class="bx-soa-section-title-val"></span>
							</div>
							<div class="bx-soa-section-edit">
								<a href="javascript:void(0)" class="bx-soa-editstep"><?=$arParams['MESS_EDIT']?></a>
							</div>
				...
				<div id="bx-soa-properties" data-visited="false" class="bx-soa-section bx-active">
					<div class="bx-soa-section-title-container">
						<div class="bx-soa-section-title">
							<span class="bx-soa-section-title-icon"><i class="icon-user"></i><i class="icon-ok-circle"></i><i class="icon-danger"></i><i class="icon-exclamation-circle"></i></span>
							<span class="bx-soa-section-title-val"><?=$arParams['MESS_BUYER_BLOCK_NAME']?></span>
						</div>
						<div class="bx-soa-section-edit">
							<a href="javascript:void(0)" class="bx-soa-editstep"><?=$arParams['MESS_EDIT']?></a>
						</div>
...
<label>
  <input type="checkbox" required>
    <span>Нажимая на эту кнопку, я даю свое согласие на обработку персональных данных и соглашаюсь с условиями <a target="_blank" href="/upload/politics-lormag.png">политики обработки персональных данных</a>.</span>
</label>
</div>
	...
	$messages["COOKIE_NAME"] = Bitrix\Main\Config\Option::get('main', 'cookie_name', 'BITRIX_SM');

	$ipAddress = Bitrix\Main\Service\GeoIp\Manager::getRealIp();
	$countryCode = Bitrix\Main\Service\GeoIp\Manager::getCountryCode($ipAddress, LANGUAGE_ID);?>
	...
	<script>
		...
		BX.Sale.OrderAjaxComponent.init({
			...
			initialCountry: '<?=CUtil::JSEscape($countryCode)?>',
			params: <?=CUtil::PhpToJSObject($arParams)?>,
			signedParamsString: '<?=CUtil::JSEscape($signedParams)?>',
			currency: '<?=CUtil::JSEscape($arResult["BASE_LANG_CURRENCY"])?>',
			siteId: '<?=CUtil::JSEscape($siteId)?>',
			defSiteId: '<?=CUtil::JSEscape(SITE_ID)?>',
			objectId: '<?=CUtil::JSEscape($objectId)?>',
	...
	<?if($arParams['SHOW_PICKUP_MAP'] === 'Y' || $arParams['SHOW_MAP_IN_PROPS'] === 'Y') {
		if($arParams['PICKUP_MAP_TYPE'] === 'yandex') {
			$this->addExternalJs($templateFolder.'/scripts/yandex_maps.min.js');
			$apiKey = htmlspecialcharsbx(Main\Config\Option::get('fileman', 'yandex_map_api_key', ''));?>
			<script src="<?=$scheme?>://api-maps.yandex.ru/2.1/?apikey=<?=$apiKey?>&lang=<?=$locale?>"></script>
			<script>
		...
		if($arParams['PICKUP_MAP_TYPE'] === 'google') {
			$this->addExternalJS($templateFolder."/scripts/infobubble.min.js");
			$this->addExternalJs($templateFolder.'/scripts/google_maps.min.js');
			$apiKey = htmlspecialcharsbx(Main\Config\Option::get('fileman', 'google_map_api_key', ''));?>
			<script async defer src="<?=$scheme?>://maps.googleapis.com/maps/api/js?key=<?=$apiKey?>&callback=bx_gmaps_waiter"></script>
			<script>
