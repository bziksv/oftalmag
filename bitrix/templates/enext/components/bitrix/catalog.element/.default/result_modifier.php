...
foreach($arResult["PROPERTIES"] as &$arProp) {
	...
	//ADVANTAGES//
	} elseif($arProp["CODE"] == "UF_ADVANTAGES") {
...
if(!empty($arResult["DISPLAY_PROPERTIES"])) {
	foreach($arResult["DISPLAY_PROPERTIES"] as &$property) {
		if($property["CODE"] == "RECOMMEND") {	
			foreach($property["VALUE"] as $codeK => $arProperty){
				$arSelect = Array("ID","NAME","PROPERTY_code");
				$arFilter = Array("IBLOCK_ID"=>IntVal($property["IBLOCK_ID"]), "XML_ID"=>$arProperty, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y");
				$res = CIBlockElement::GetList(Array(), $arFilter, false, Array("nPageSize"=>1), $arSelect);
				while($ob = $res->GetNextElement()){ 
					$arFields = $ob->GetFields();  	
					if($arFields["PROPERTY_CODE_VALUE"]){
						$property["DISPLAY_VALUE"][$codeK] = '<a href="/product/'.$arFields["PROPERTY_CODE_VALUE"].'/">'.$arFields["NAME"].'</a>';
					}
				}
			}
			$property["DISPLAY_VALUE"] = is_array($property["DISPLAY_VALUE"]) ? implode(" / ", $property["DISPLAY_VALUE"]) : strip_tags($property["DISPLAY_VALUE"]);
		} elseif($property["CODE"] == "BRAND") {
