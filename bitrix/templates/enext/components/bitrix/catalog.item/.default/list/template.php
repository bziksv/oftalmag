...
if($item["PROPERTIES"]["code"]["VALUE"]) {
	...
	foreach($item['PROPERTIES'] as $prop) {
		if($prop['CODE'] == 'preimushchestva') {

			if(empty($prop['VALUE'])) {


			foreach($arResult["SECTION"]['SECTION_PATH'] as $arSectionPath) {
...
<div class="product-item">
	<div class="product-item-image-wrapper" data-entity="image-wrapper">		
		...
		<a target="_self" class="product-item-image" id="<?=$itemIds['PICT_ID']?>" href="<?=$item['DETAIL_PAGE_URL']?>" title="<?=$imgTitle?>"<?=($arParams["QUICK_VIEW"] == "FULL" ? " data-entity='quickView'" : "")?>>
			...
			if(!empty($item['PROPERTIES']['BRAND']['FULL_VALUE']['PREVIEW_PICTURE'])) {?>
				<div class="hidden-xs hidden-sm product-item-brand">
					<img src ="/bitrix/images/arturgolubev.lazyload/pixel.gif" data-lazyload-src="<?=$item['PROPERTIES']['BRAND']['FULL_VALUE']['PREVIEW_PICTURE']['SRC']?>" width="<?=$item['PROPERTIES']['BRAND']['FULL_VALUE']['PREVIEW_PICTURE']['WIDTH']?>" height="<?=$item['PROPERTIES']['BRAND']['FULL_VALUE']['PREVIEW_PICTURE']['HEIGHT']?>" alt="<?=$item['PROPERTIES']['BRAND']['FULL_VALUE']['NAME']?>" title="<?=$item['PROPERTIES']['BRAND']['FULL_VALUE']['NAME']?>" />
				</div>
			<?}?>
		</a>
		<?//DELAY//
		...
		if($arParams['QUICK_VIEW'] != 'OFF') {?>
			<div class="hidden-xs hidden-sm product-item-quick-view"<?=($arParams["QUICK_VIEW"] == "FULL" ? " data-entity='quickView'" : " id='".$itemIds['QUICK_VIEW_LINK']."'")?>><i class="icon-eye"></i><span><?=Loc::getMessage('CT_BCI_TPL_MESS_QUICK_VIEW')?></span></div>
		<?}?>
	</div>
	<div class="product-item-info">
		...
		if((!$object || ($object && $objectContacts)) && !$partnersUrl && $haveOffers && ($arParams['OFFERS_VIEW'] == 'PROPS' || $arParams['OFFERS_VIEW'] == 'DROPDOWN_LIST') && $arParams['PRODUCT_DISPLAY_MODE'] == 'Y' && !empty($item['OFFERS_PROP'])) {?>
			<div class="hidden-xs hidden-sm" id="<?=$itemIds['TREE_ID']?>">
		...
		if((!$object || ($object && $objectContacts)) && !$partnersUrl && !$haveOffers) {
			if($arParams['ADD_PROPERTIES_TO_BASKET'] == 'Y' && !empty($item['PRODUCT_PROPERTIES'])) {?>
				<div class="hidden-xs hidden-sm" id="<?=$itemIds['BASKET_PROP_DIV']?>">
		...
		if(!empty($item['PREVIEW_TEXT'])) {?>
			<div class="hidden-xs hidden-sm product-item-preview-text"><?=$item['PREVIEW_TEXT']?></div>
		<?}
		//PROPERTIES//
		if(!empty($item['DISPLAY_PROPERTIES']) || $item['OFFERS_PROPS_DISPLAY']) {?>
			<div class="hidden-xs hidden-sm product-item-properties-block"<?=((!$object || ($object && $objectContacts)) && !$partnersUrl && $haveOffers && ($arParams['OFFERS_VIEW'] == 'PROPS' || $arParams['OFFERS_VIEW'] == 'DROPDOWN_LIST') && $arParams['PRODUCT_DISPLAY_MODE'] == 'Y' && $item['OFFERS_PROPS_DISPLAY'] ? ' id="'.$itemIds['DISPLAY_PROP_DIV'].'"' : '')?>>
				<?foreach($item['DISPLAY_PROPERTIES'] as $property) {?>										
					<div class="product-item-properties">
						<div class="product-item-properties-name">
							<? if($property['NAME'] === "Бренд"): ?>
								<? if(isWrapAttr($arResult["COUNTER"], $arResult["SECTION"]["UF_BRAND_ATTR"])): ?>
									<?/*<span show-content="<?=$property['NAME']?>"></span>*/?>
									<?=$property['NAME']?>
								<? else: ?>
									<span class="show_brand"></span>
						...
						<div class="product-item-properties-val"><?=$property['DISPLAY_VALUE']?></div>
					</div>
				<?}
				unset($property);
				if($item['OFFERS_PROPS_DISPLAY']) {
					foreach($actualItem['DISPLAY_PROPERTIES'] as $property) {?>
						<div class="product-item-properties" data-entity="sku-props">
							<div class="product-item-properties-name"><?=$property['NAME']?></div>
							<div class="product-item-properties-val"><?=(is_array($property['VALUE']) ? implode(' / ', $property['VALUE']) : $property['VALUE'])?></div>
						</div>
					<?}
					unset($property);
				}?>
		...
		<?}
		//ADVANTAGES//			
		if(!empty($item['PROPERTIES']['ADVANTAGES']['FULL_VALUE'])) {?>
			<div class="hidden-xs hidden-sm product-item-advantages">
				<?foreach($item['PROPERTIES']['ADVANTAGES']['FULL_VALUE'] as $arAdv) {
					if(!empty($arAdv["PREVIEW_PICTURE"])) {?>
						<div class="product-item-advantages-item">
							<div class="product-item-advantages-item-pic">
								<img src ="/bitrix/images/arturgolubev.lazyload/pixel.gif" data-lazyload-src="<?=$arAdv['PREVIEW_PICTURE']['SRC']?>" width="<?=$arAdv['PREVIEW_PICTURE']['WIDTH']?>" height="<?=$arAdv['PREVIEW_PICTURE']['HEIGHT']?>" alt="<?=$arAdv['NAME']?>" />
							</div>
							<div class="visible-md visible-lg product-item-advantages-item-tooltip"><?=$arAdv["NAME"]?></div>
						</div>
					<?}
				}
				unset($arAdv);?>
					...
					<?if(!empty($price)) {
						if($haveOffers && (($object && !$objectContacts) || $partnersUrl || ($arParams['OFFERS_VIEW'] != 'PROPS' && $arParams['OFFERS_VIEW'] != 'DROPDOWN_LIST') || $arParams['PRODUCT_DISPLAY_MODE'] == 'N')) {
							if(($arParams['OFFERS_VIEW'] == 'LIST' && $price['SQ_M_PRICE'] > 0) || $price['PRICE'] > 0) {?>
								...
								<? if(isWrapAttr($arResult["COUNTER"], $arResult["SECTION"]["UF_PRICE_ATTR"])): ?>
									<span class="product-item-price-not-set"><?=Loc::getMessage('CT_BCI_TPL_MESS_PRICE_NOT_SET')?></span>
								<? else: ?>
									<span class="product-item-price-not-set show_price"></span>
							...
							<? if(isWrapAttr($arResult["COUNTER"], $arResult["SECTION"]["UF_PRICE_ATTR"])): ?>
								<span class="product-item-price-not-set" data-entity="price-current-not-set"<?=($price['SQ_M_PRICE'] > 0 ? ' style="display:none;"' : ($price['PRICE'] > 0 ? ' style="display:none;"' : ''))?>><?=Loc::getMessage('CT_BCI_TPL_MESS_PRICE_NOT_SET')?></span>
							<? else: ?>
								<span class="product-item-price-not-set show_price" <?=($price['SQ_M_PRICE'] > 0 ? ' style="display:none;"' : ($price['PRICE'] > 0 ? ' style="display:none;"' : ''))?>></span>
							<? endif;?>
...
*/?>							
					<div class="hidden-xs hidden-sm" id="<?=$itemIds['QUANTITY_LIMIT_NOT_AVAILABLE']?>" style="display: none;">
						<div class="product-item-quantity product-item-quantity-not-avl">
							...
							<span class="product-item-quantity-val">11<?=$arParams['MESS_NOT_AVAILABLE']?></span>
						</div>
					</div>
				<?} else {?>
					<div class="hidden-xs hidden-sm" id="<?=$itemIds['QUANTITY_LIMIT']?>">
						<div class="product-item-quantity<?=($actualItem['CAN_BUY'] ? '' : ' product-item-quantity-not-avl')?>">
							...
							<? if(isWrapAttr($arResult["COUNTER"], $arResult["SECTION"]["UF_AVAILABLE_ATTR"])): ?>
								<span class="product-item-quantity-val 222">
							...
							<? else: ?>
								<span class="product-item-quantity-val 33">
									<?if($actualItem['CAN_BUY']) {?>
									<?/*<span class="product-item-quantity-val" show-content="<?=$arParams['MESS_SHOW_MAX_QUANTITY'].'&nbsp;'?>"></span>*/?>
<?/*
									<span class="product-item-quantity-val show_in_stock">&nbsp;</span>
*/?>
										<?if($measureRatio && (float)$actualItem['CATALOG_QUANTITY'] > 0 && $actualItem['CATALOG_QUANTITY_TRACE'] == 'Y' && $actualItem['CATALOG_CAN_BUY_ZERO'] == 'N') {
											if($arParams['SHOW_MAX_QUANTITY'] == 'M') {
												if((float)$actualItem['CATALOG_QUANTITY'] / $measureRatio >= $arParams['RELATIVE_QUANTITY_FACTOR']) {
		...
		<div class="product-item-block">
			...
			//TOTAL_COST//
			if($arParams['USE_PRODUCT_QUANTITY'] && (!$object || ($object && $objectContacts)) && !$partnersUrl && ((!$haveOffers && $actualItem['CAN_BUY']) || ($haveOffers && ($arParams['OFFERS_VIEW'] == 'PROPS' || $arParams['OFFERS_VIEW'] == 'DROPDOWN_LIST') && $arParams['PRODUCT_DISPLAY_MODE'] == 'Y'))) {?>
				<div class="hidden-xs hidden-sm product-item-total-cost" id="<?=$itemIds['TOTAL_COST_ID']?>"<?=($price['MIN_QUANTITY'] != 1 || (!empty($item['PROPERTIES']['M2_COUNT']['VALUE']) && ($price['PC_MIN_QUANTITY'] != 1 || $price['SQ_M_MIN_QUANTITY'] != 1)) ? '' : ' style="display:none;"')?>>
					<?if(isWrapAttr($arResult["COUNTER"], $arResult["SECTION"]["UF_COMMON_PRICE_ATTR"])):?>
						<?=Loc::getMessage('CT_BCI_TPL_MESS_TOTAL_COST')?>
					<? else: ?>
						<span class="show_total_price"></span>
						
					<? endif; ?>
					<span data-entity="total-cost">
						<?=($price['MIN_QUANTITY'] != 1 || (!empty($item['PROPERTIES']['M2_COUNT']['VALUE']) && ($price['PC_MIN_QUANTITY'] != 1 || $price['SQ_M_MIN_QUANTITY'] != 1)) ? $price['PRINT_RATIO_PRICE'] : '')?>
					</span>
				</div>
			<?}
			//BUTTONS//?>
				...
				<?} else {
					if(!$haveOffers) {
						...
						<?} elseif($actualItem['CAN_BUY'] && $price['RATIO_PRICE'] <= 0 && $arParams['ASK_PRICE']) {?>
							<button type="button" class="btn btn-default" id="<?=$itemIds['ASK_PRICE_LINK']?>" title="<?=Loc::getMessage('CT_BCI_TPL_MESS_ASK_PRICE')?>"><i class="icon-comment"></i>
								
					<?if(isWrapAttr($arResult["COUNTER"], $arResult["SECTION"]["UF_REQUEST_PRICE_ATTR"])):?>
						<span><?=Loc::getMessage('CT_BCI_TPL_MESS_ASK_PRICE')?></span>
					<? else: ?>
						<span class="show_request_price"></span>
					<? endif; ?>
						...
						<?if($arParams['ASK_PRICE']) {?>
							<button type="button" class="btn btn-default" id="<?=$itemIds['ASK_PRICE_LINK']?>" title="<?=Loc::getMessage('CT_BCI_TPL_MESS_ASK_PRICE')?>"<?=(!$offerPartnersUrl && $actualItem['CAN_BUY'] && $price['RATIO_PRICE'] <= 0 ? '' : ' style="display: none;"')?>><i class="icon-comment"></i>
					<?if(isWrapAttr($arResult["COUNTER"], $arResult["SECTION"]["UF_REQUEST_PRICE_ATTR"])):?>
						<span><?=Loc::getMessage('CT_BCI_TPL_MESS_ASK_PRICE')?></span>
					<? else: ?>
						<span class="show_request_price"></span>
